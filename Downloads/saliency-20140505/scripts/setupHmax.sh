#!/bin/bash
# Script to automate prepping and running of HMAX CPU code
# ./setupHmax.sh <input_movie> <labels_file> [<test_movie>]
...
"$BASE/saliency/bin/neovision2" $LOCIP:$LOCALPORT --in=movie:$INPUTMOV --out=display: > /dev/null 2>&1
...
mkdir -p $C1PATCHES
"$BASE/saliency/bin/extractpatches" $C1PATCHES "$SAMPLEDIR" > /dev/null 2>&1
if [ $? -ne 0 ];  then
...
# 5) Run HMAX in Batch Mode to Get Example Feature Arrays to train the SVM
rm -f $DD/train.out
rm -f $DD/test.out
while read line
do
ID=`echo $line | awk '{print $1}' `;
LABEL=`echo $line | awk '{print $2}' `;
$BASE/saliency/bin/runhmaxfl $C1PATCHES dir:"$DD/Train/$LABEL/" $ID "$DD/$LABEL.out" > /dev/null 2>&1
if [ $? -ne 0 ];  then
...
rm -f $DD/$LABEL.out
$BASE/saliency/bin/runhmaxfl $C1PATCHES dir:"$DD/Test/$LABEL/" $ID "$DD/$LABEL.out" > /dev/null 2>&1
if [ $? -ne 0 ];  then
...
"$BASE/saliency/bin/hmaxfl-server" $LABELFILE $C1PATCHES $FEATURESFILE $LOCALPORT $SRVIP $SRVPORT $SVMMODEL $SVMRANGE &
HMSPID=$!
# 7.2) Run neovision
"$BASE/saliency/bin/neovision2" $LOCIP:$LOCALPORT --in=movie:$TESTMOV --out=display:
# 7.3) Extraction complete, end the save image server
kill $HMSPID

# 7bis) Alternatively, run HMAX Batch Tester 
#while read line
#do
#ID=`echo $line | awk '{print $1}' `;
#LABEL=`echo $line | awk '{print $2}' `;
#$BASE/saliency/bin/testhmaxfl $C1PATCHES dir:$DD/Test/$LABEL/ $SVMMODEL $SVMRANGE $LABEL.test.out
#done < $LABELFILE
