...
# bzip2) into a .tbz archive of PPM frames for use, for example, with
# process_movie.pl.
#
...
#
# USAGE: movie2tbz.pl [--slow] [--mpg] [--mpghq] [--fbeg=frame] [--fend=frame]
...
# a factor 2. Use several --slow if one is not sufficient. If --mpg is
# specified, an MPEG-1 version of the movie will ve created in addition to
# the .tbz version. If --mpghq is specified, a high-quality MPEG-1 encoder
...
## $Id: movie2tbz.pl 10337 2008-10-13 17:24:09Z itti $
##
...
# parse command line:
$speed = 1.0; $do_mpg = 0; $do_mpghq = 0; $fbeg = 0; $fend = 2000000000;
foreach $a (@ARGV) {
    if ($a eq '--slow') { $speed *= 0.5; }
    elsif ($a eq '--mpg') { $do_mpg = 1; }
    elsif ($a eq '--mpghq') { $do_mpghq = 1; }
...
$cmd_extract = "mplayer -vo png -nosound -benchmark -nojoystick ".
    "-nolirc -cache 8192 -speed $speed -noframedrop ";
    # may sometimes need to force aspect:
    # "-monitoraspect 1.5 -aspect 1.5";
$cmd_conv = "pngtopnm";
...
foreach $f (@ARGV) {
    ...
    # create a quick mpeg-1?
    if ((! -f $mname) && ($do_mpg || $do_mpghq)) {
