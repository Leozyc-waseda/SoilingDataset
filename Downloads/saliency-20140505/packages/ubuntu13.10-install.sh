...
# Installation guide for Ubuntu 13.04 amd64
...
# The swap file should not be readable by normal users, otherwise they could snoop on the memory of other user's
# processes:
...
# Check if it worked
#---    cat /proc/meminfo
# Determine old swap partition device - only do this if you previously had a swap partition:
#---    sudo fdisk -l /dev/sdb
# Decommission the old swap partition (your specific partition will vary):
#---    sudo swapoff /dev/sdb6
#
# At this point there is some housekeeping to do in /etc/fstab file to ensure the changes persist on the next
# boot. Remove the line for the old swap partition and add the following for #the new swap file:
#
#/mnt/8GiB.swap  none            swap    sw              0       0
#
# If using an SSD drive, try the following: Now we want to balance the fact that the swap file is on the SSD with the
# desire to reduce writes to the SSD to prolong the life of the drive. (Although I am of the opinion that such #concerns
# are overblown, I like the effect of this change anyway.) We will tell the system to prefer RAM over swap using the
# swappiness setting. Add (or edit) the following in /etc/sysctl.conf
#
# vm.swappiness=1
# vm.vfs_cache_pressure=50
#
# Since we have a mixed environment, we cannot just set it system wide via grub kernel options. That is, we can’t just
# use the elevator or deadline algorithm for all disks since we have #both HDDs and SSDs. In order to use the correct
# algorithm with the proper drive type, create a file in /etc/udev/rules.d with contents:
#
...
#
# I named mine 99-scheduler.rules. The files in this directory are automatically processed by udev, the device manager
# for the kernel. Note that cfq, or Completely Fair Queuing, is the #default. For more information, go to
...
deb http://mirrors.usc.edu/pub/linux/distributions/ubuntu/ saucy main restricted universe multiverse
deb http://mirrors.usc.edu/pub/linux/distributions/ubuntu/ saucy-updates main restricted universe multiverse
deb http://mirrors.usc.edu/pub/linux/distributions/ubuntu/ saucy-backports main restricted universe multiverse
deb http://mirrors.usc.edu/pub/linux/distributions/ubuntu/ saucy-security main restricted universe multiverse
...
# TODO: should be updated for lightdm
# turn off dm:
#read -p "Do you want to turn off gdm (will take effect after reboot) (y/N)? "
#if [ "X$REPLY" = "Xy" ]; then
...
### Note: we will install ice3.5 as ice3.4 has issues with the latest g++
...
#    apt-get -y --force-yes install icee-slice icee-translators icegrid-gui
    ...
    apt-get -y --force-yes install libboost1.54-all-dev  # boost
