...
if [ ! -d /lab/packages/forall ]; then
	...
	# and finally mv /usr/local/lib/liblwpr* /usr/lib
        ...
        tar jxpf /lab/vault/software/epix/xclib-installed-x64-ubuntu.tbz
	echo "Installing xv..."
...
blacklist rivatv
EOF
	    /lab/packages/forall/current/devdriver_3.1_linux_64_256.40.run
	fi

	read -p "Do you want to install CUDA dev kit (ok even if you don't have a CUDA board) (Y/n)? "
	echo "For the cuda toeolkit, select /usr/local/cuda for installation."
	echo "For the cuda SDK, select /usr/local/cudasdk for installation."
	if [ "X$REPLY" != "Xn" ]; then
	    /lab/packages/forall/current/cudatoolkit_3.1_linux_64_ubuntu9.10.run
	    /lab/packages/forall/current/gpucomputingsdk_3.1_linux.run
	fi
    fi
fi
