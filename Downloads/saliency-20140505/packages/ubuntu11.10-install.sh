...
# Installation guide for Ubuntu 11.10 amd64
...
deb http://mirrors.usc.edu/pub/linux/distributions/ubuntu/ oneiric main restricted universe multiverse
deb http://mirrors.usc.edu/pub/linux/distributions/ubuntu/ oneiric-updates main restricted universe multiverse
deb http://mirrors.usc.edu/pub/linux/distributions/ubuntu/ oneiric-backports main restricted universe multiverse
deb http://mirrors.usc.edu/pub/linux/distributions/ubuntu/ oneiric-security main restricted universe multiverse
...
if [ "X$REPLY" != "Xn" ]; then
  ...
  apt-get -y install gtk2-engines-pixbuf # for annoying error msgs
...
if [ "X$REPLY" != "Xn" ]; then
    ...
    apt-get -y install tcl8.4-dev
    apt-get -y install tk8.4-dev
    ...
    apt-get -y install libgsl0ldbl-dev
    ...
    apt-get -y install zeroc-ice34
    apt-get -y install ice34-services icee-slice ice34-translators 
    apt-get -y install libzeroc-ice34 libicee-dev libslice34
    apt-get -y install libzeroc-ice34-dev
    ...
    apt-get -y install chkconfig  # to configure services
...
if [ "X$REPLY" = "Xy" ]; then
    echo "deb http://packages.ros.org/ros/ubuntu oneiric main" > /etc/apt/sources.list.d/ros-latest.list
...
if [ "X$REPLY" != "Xn" ]; then
    add-apt-repository "deb http://nrtkit.org/apt oneiric main"
    apt-get update
    apt-get install nrt
...
if [ ! -d /lab/packages/forall ]; then
    ...
    if  [ -d /lab/local/matlabR2010b/bin/glnxa64 ]; then
	echo /lab/local/matlabR2010b/bin/glnxa64 > /etc/ld.so.conf.d/zzzmatlab.conf
    fi

    if  [ -d /usr/local/matlabR2011a/bin/glnxa64 ]; then
	echo /lab/local/matlabR2010b/bin/glnxa64 > /etc/ld.so.conf.d/zzzmatlab.conf
    fi
    ldconfig
