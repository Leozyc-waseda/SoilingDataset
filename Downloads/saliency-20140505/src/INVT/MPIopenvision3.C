/*!@file INVT/MPIopenvision3.C  MPI version of openvision3.C */
...
// $HeadURL: svn://isvn.usc.edu/software/invt/trunk/saliency/src/INVT/MPIopenvision3.C $
// $Id: MPIopenvision3.C 10845 2009-02-13 08:49:12Z itti $
//
...
//                 }
  ...
  else
    {
      ...
      for (int i = 0; i < NB_PICS; i++)
        {
          input[i] =
            Raster::ReadRGB(sformat("%s/pictures/sat_%03i.ppm", PATH, i));
          int count = 0;
          std::ifstream chfile(sformat("%s/pictures/sat_%03i.ch.dat", PATH, i).c_str());
          bool eofile = false;
          while (!chfile.eof())
            {
              int px, py;
          ...
          nch[i] = count;
          count = 0;
          std::ifstream kpfile(sformat("%s/pictures/sat_%03i.kp.dat", PATH, i).c_str());
          eofile = false;
          while (!eofile)
            {
              int px, py;
          ...
          nkp[i] = count;
          count = 0;
          std::ifstream pwfile(sformat("%s/pictures/sat_%03i.pw.dat", PATH, i).c_str());
          eofile = false;
          while (!eofile)
            {
              int px, py;
          ...
          npw[i] = count;
          count = 0;
          std::ifstream wkmfile(sformat("%s/pictures/sat_%03i.wkm.dat", PATH, i).c_str());
          eofile = false;
          while (!eofile)
            {
              int px, py;
