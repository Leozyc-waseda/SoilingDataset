...
// $HeadURL: svn://isvn.usc.edu/software/invt/trunk/saliency/src/NeovisionII/ObjDet/SaliencyObjDetBrain.C $
// $Id: SaliencyObjDetBrain.C 12962 2010-03-06 02:13:53Z irock $
//
...
class SaliencyBrain : public ObjDetBrain
{

  public:
  SaliencyBrain( const int argc, const char **argv) :
    itsArgc(argc),
    itsArgv(argv)
  {
  ...
  std::vector<DetLocation> onDetection(Image<PixRGB<byte> > &img)
  {
        ...
        img.getHeight() > 0)
    {
      ...
      nub::ref<GetSaliency> itsVcc(new GetSaliency(manager));
      ...
      Image<float> vcxMap = itsVcc->getVCXmap(img);
  ...
  private:
    //ModelManager* itsMgr;
    //nub::soft_ref<GetSaliency> itsVcc;
    const int itsArgc;
    const char** itsArgv;
...
{
  return new SaliencyBrain(argc, argv);
}
