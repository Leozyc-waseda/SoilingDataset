...
// $HeadURL: svn://isvn.usc.edu/software/invt/trunk/saliency/src/NeovisionII/ObjDet/RandomObjDetBrain.C $
// $Id: RandomObjDetBrain.C 12962 2010-03-06 02:13:53Z irock $
//
...
class RandomBrain : public ObjDetBrain
{
  ...
  std::vector<DetLocation> onDetection(Image<PixRGB<byte> > &img)
  {
    ...
    {
      int n = randomUpToNotIncluding(smap.size());
      float tmp = smap[i].val;
      smap[i].val = smap[n].val;
      smap[n].val = tmp;
    }
...
{
  return new RandomBrain(argc, argv);
}
