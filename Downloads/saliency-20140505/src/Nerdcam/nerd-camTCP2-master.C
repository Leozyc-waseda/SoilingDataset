/*!@file Nerdcam/nerd-camTCP2-master.C Grab & process over beowulf w/ pvisionTCP2 */
...
// $HeadURL: svn://isvn.usc.edu/software/invt/trunk/saliency/src/Nerdcam/nerd-camTCP2-master.C $
// $Id: nerd-camTCP2-master.C 9412 2008-03-10 23:10:15Z farhan $
//
...
int main(const int argc, const char **argv)
{
  ...
  XWindow xw(Dims(w * 2, h), -1, -1, "USC Nerd Cam");
  ...
  int lastX = w/2; int lastY = h/2;// keep track of last secade
  int overTravel = 0;
  float move = 0;
  float delay = 0;
  ...
  while(goforever)
    {
      ...
      if(move <= 0)
      {
        ...
        LINFO("CAMERA MOVE %d, %d",modi,modj);
        camPause.reset();
        LINFO("travel time is %f",delay);

        if((lastX >= 0) && (lastX < w) && (lastY >= 0) && (lastY < h))
        {
          drawCircle(disp, Point2D<int>(lastX,lastY), foa_size, pix, 1);
          drawArrow(disp, Point2D<int>(lastX,lastY),win,pix,1);
        }
        if(delay < 0)
        {
          writeText(disp, Point2D<int>(50,50),"666");
          overTravel++;
          LINFO("OVERTRAVEL %d",overTravel);
          delay = camera->moveCamXYFrame(320,240);
          LINFO("CAMERA MOVE %d, %d",320,240);
          camPause.reset();
          //delay = 0;
        }
        move = delay - camPause.get();
      }
      else
      {
        move = delay - camPause.get();
      ...
      if (sm.initialized())
        inplacePaste(disp, toRGB(quickInterpolate(sm, 1<<sml)), Point2D<int>(w, 0));
      xw.drawImage(disp);
