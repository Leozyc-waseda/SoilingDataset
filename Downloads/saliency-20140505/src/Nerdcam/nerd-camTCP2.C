/*!@file Nerdcam/nerd-camTCP2.C A parallel vision slave to use w/ pvisionTCP2master */
...
// $HeadURL: svn://isvn.usc.edu/software/invt/trunk/saliency/src/Nerdcam/nerd-camTCP2.C $
// $Id: nerd-camTCP2.C 12074 2009-11-24 07:51:51Z itti $
//
...
int main(const int argc, const char **argv)
{
  ...
  ModelManager manager("NerdCam TCP - Slave");
      ...
      if (beo->receive(rnode, rmsg, rframe, raction, 3))  // wait up to 3ms
        {
                    ...
                    if (nbcmap[curridx])
                      {
                        ...
                        char junk[1024], junk2[256]; junk[0] = '\0';
                        for (int i = 0; i < NBOUT; i ++)
                          {
                            sprintf(junk2, "%d/%d ", outframe[i], nbcmap[i]);
                            strcat(junk, junk2);
                          }
                        LINFO("BUFFER = [ %s]", junk);
                      }
          ...
          if (rframe % 101 == 0)
            LINFO("Action %d, frame %d processed in %llxms",
...
{
  // compute pyramid:
  ImageSet<float> pyr = buildPyrGeneric(fima, 0, maxdepth,
                                        ptyp, ori);
