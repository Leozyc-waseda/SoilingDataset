/*!@file AppPsycho/psycho-3D-task-stimulus-classification.C psychopysics experiment for classifying the task and stimulus based on eye-movement data */
...
// $HeadURL: svn://isvn.usc.edu/software/invt/trunk/saliency/src/App3DEye/psycho-3D-task-stimulus-classification.C $
//
...
string get2301(string s){
        string t = "" ;
        t = s.substr(2,1)+s.substr(3,1)+s.substr(0,1)+s.substr(1,1);
        return t;
}


string get1032(string s){
        string t="";
        t=s.substr(1,1)+s.substr(0,1)+s.substr(3,1)+s.substr(2,1);
        return t;
}
...
void displayLinear(string s , int onsetTime, int ws ){
        int x = (d->getWidth()-(s.size()-1)*ws)/2 ;
        int y = d->getHeight()/2 - 10;
        for( uint k = 0 ; k < s.size() ; k++ ){
                d->displayText(s.substr(k,1),Point2D<int>(x+k*ws,y),PixRGB<byte>(0,0,0),PixRGB<byte>(128,128,128),true) ;
                d->waitFrames(onsetTime);
                d->clearScreen() ;
        }
}
...
vector<string> getAll_34012(string al="0123456789"){
            string tmp;
            vector<string> outVector ;
            for(int i0=0 ;i0<10 ;i0++ ){
                for( int i1=i0+2;i1<10 ; i1++ ){
                    for( int i2=i1+2 ;i2<10 ; i2++){
                        for( int i3=i2+1  ; i3 <10 ; i3++ ){
                            for( int i4=i3+2; i4 < 10 ; i4++){
                                tmp = al.substr(i3,1)+al.substr(i4,1)+al.substr(i0,1)+al.substr(i1,1)+al.substr(i2,1);
...
vector<string> getAll_21043(string al="0123456789"){
            string tmp;
            vector<string> outVector ;
            for(int i0=0 ;i0<10 ;i0++ ){
                for( int i1=i0+2;i1<10 ; i1++ ){
                    for( int i2=i1+2 ;i2<10 ; i2++){
                        for( int i3=i2+1  ; i3 <10 ; i3++ ){
                            for( int i4=i3+2; i4 < 10 ; i4++){
                                tmp = al.substr(i2,1)+al.substr(i1,1)+al.substr(i0,1)+al.substr(i4,1)+al.substr(i3,1);
                                outVector.push_back(tmp);
                            }
                        }
                    }
...
vector<string> getAll_41230(string al="0123456789"){
            string tmp;
            vector<string> outVector ;
            for(uint i0=0 ;i0<al.size() ;i0++ ){
                for( uint i1=i0+1;i1<al.size() ; i1++ ){
                    for( uint i2=i1+2 ;i2<al.size() ; i2++){
                        for( uint i3=i2+2  ; i3 <al.size() ; i3++ ){
                            for( uint i4=i3+1; i4 < al.size() ; i4++){
                                tmp = al.substr(i4,1)+al.substr(i1,1)+al.substr(i2,1)+al.substr(i3,1)+al.substr(i0,1);
                                outVector.push_back(tmp);
                            }
                        }
                    }
...
vector<string> getAll_03214(string al="0123456789"){
            string tmp;
            vector<string> outVector ;
            for(uint i0=0 ;i0<al.size() ;i0++ ){
                for( uint i1=i0+1;i1<al.size() ; i1++ ){
                    for( uint i2=i1+2 ;i2<al.size() ; i2++){
                        for( uint i3=i2+2  ; i3 <al.size() ; i3++ ){
                            for( uint i4=i3+1; i4 < al.size() ; i4++){
                                tmp = al.substr(i0,1)+al.substr(i3,1)+al.substr(i2,1)+al.substr(i1,1)+al.substr(i4,1);
                                outVector.push_back(tmp);
                            }
                        }
                    }
...
vector<string> getAll_2301(string al="0123456789"){
  string tmp;
  vector<string> outVector;
  for(uint i0=0 ;i0<al.size() ;i0++ ){
                for( uint i1=i0+1;i1<al.size() ; i1++ ){
                    for( uint i2=i1+2 ;i2<al.size() ; i2++){
                        for( uint i3=i2+2  ; i3 <al.size() ; i3++ ){
                              tmp = al.substr(i2,1)+al.substr(i3,1)+al.substr(i0,1)+al.substr(i1,1);
                              outVector.push_back(tmp);
                        }
                    }
                }
...
vector<string> getAll_1032(string al="0123456789"){
  string tmp;
  vector<string> outVector;
  for(uint i0=0 ;i0<al.size() ;i0++ ){
                for( uint i1=i0+1;i1<al.size() ; i1++ ){
                    for( uint i2=i1+2 ;i2<al.size() ; i2++){
                        for( uint i3=i2+2  ; i3 <al.size() ; i3++ ){
                              tmp = al.substr(i1,1)+al.substr(i0,1)+al.substr(i3,1)+al.substr(i2,1);
                              outVector.push_back(tmp);
                        }
                    }
                }
...
extern "C" int main(const int argc, char** argv)
{
        ...
        argMap["white-space"] = "30" ;
        argMap["cue-type"] = "v" ;
        argMap["sound-dir"]="..";
        argMap["num-of-a-sorting-trials"] = "2" ;
        argMap["num-of-d-sorting-trials"] = "2" ;
        argMap["num-of-mem-trials"] = "2" ;
        argMap["first-stop"]="20";
        argMap["second-stop"]="40";
        argMap["third-stop"] = "60";
        argMap["asc-cue-sound-file"]="sine.wav";
        argMap["desc-cue-sound-file"]="square.wav";
        manager.addSubComponent(d);
        nub::soft_ref<EventLog> el(new EventLog(manager));
        manager.addSubComponent(el);
	nub::soft_ref<LEDcontroller> itsledcon(new LEDcontroller(manager));
  ...
  // let's display an ISCAN calibration grid:
        ...
        //let's see in what mode the user like to run the program
            ...
            int num_of_a_sorting_task = atoi(argMap["num-of-a-sorting-trials"].c_str()) ;
            int num_of_d_sorting_task = atoi(argMap["num-of-d-sorting-trials"].c_str()) ;
            int num_of_mem_task = atoi(argMap["num-of-mem-trials"].c_str()) ;
            int first_stop = atoi(argMap["first-stop"].c_str());
            int second_stop = atoi(argMap["second-stop"].c_str());
            int third_stop = atoi(argMap["third-stop"].c_str());
            string alphabet = argMap["alphabet"];
            //let's do calibration
            d->displayText("CLICK LEFT button to calibrate; RIGHT to skip");
            int cl = d->waitForMouseClick();
            if (cl == 1) eyet->calibrate(d);
            d->clearScreen() ;
            d->displayText("CLICK LEFT button for off-screen clibration; RIGHT to skip");
            cl = d->waitForMouseClick();
            //now time for offscreen calibration
            if (cl==1){
            for(int i=1;i<5;i++){
                d->clearScreen();
            ...
            string str = argMap["sound-dir"]+"/"+argMap["asc-cue-sound-file"] ;
            recallCueMusic = Mix_LoadMUS(str.c_str());
            if(recallCueMusic==NULL) cout<<"not a 16 bit wav file  "<<str<<endl;
            str = argMap["sound-dir"]+"/"+argMap["desc-cue-sound-file"] ;
            sortCueMusic = Mix_LoadMUS(str.c_str());
            if(sortCueMusic==NULL) cout<<"not a 16 bit wav file  "<<str<<endl;
            if(argMap["cue-type"].compare("a")==0){
                     //now let's open the audio channel
                                                for(uint i = 0; i < alphabet.size() ; i++){
                                                        string str = argMap["sound-dir"]+"/"+alphabet.substr(i,1)+".wav";
                                                        audio_map[i]=Mix_LoadMUS(str.c_str());
                                                        if(audio_map[i]==NULL) cout<<"not a 16 bit wav file  "<<str<<endl;
                                                        charmap[alphabet.substr(i,1)]=i;
                                                        charinvmap[i]=alphabet.substr(i,1) ;
                                                }
            }
            vector<int>* taskVector = new vector<int>();
            for(int i = 0 ; i < (int)(num_of_a_sorting_task/2) ; i++) {
                    taskVector->push_back(1);
                    taskVector->push_back(2);
                }
            for(int i = 0 ; i < (int)(num_of_d_sorting_task/2) ; i++) {
            taskVector->push_back(3);
            taskVector->push_back(4);
            }
            
            vector<string> a_41230 = getAll_41230(alphabet);
            vector<string> a_03214 = getAll_03214(alphabet);
            vector<string> d_41230 = getAll_41230(alphabet);
            vector<string> d_03214 = getAll_03214(alphabet);
            scramble(a_41230);
            scramble(a_03214);
            scramble(d_41230);
            scramble(d_03214);
            ...
            /*
            vector<string> a_2301 = getAll_2301(alphabet);
            vector<string> a_1032 = getAll_1032(alphabet);
            vector<string> d_2301 = getAll_2301(alphabet);
            vector<string> d_1032 = getAll_1032(alphabet);
            scramble(a_2301);
            scramble(a_1032);
            scramble(d_2301);
            scramble(d_1032);
            */
            ...
            for( int r = 0 ; (int)r < (int)taskVector->size() ; r++ ){
                            ...
                            if(r==first_stop || r==second_stop || r==third_stop){
                                d->displayText("YOU MAY STOP HERE TO TAKE A SHORT BRAKE!");
                                d->waitForMouseClick();
                                d->displayText("incorrect memory recalls="+stringify(incorrectMemory)+"    incorrect ascending sorting:"+stringify(incorrectAsc) + "  incorrect descencing sorting"+ stringify(incorrectDes) );
                                d->waitForMouseClick();
                                d->displayText("CLICK LEFT button to calibrate; RIGHT to skip");
                                int cl = d->waitForMouseClick();
                                if (cl == 1) eyet->calibrate(d);;
				...
				d->displayText("CLICK LEFT button for off-screen clibration; RIGHT to skip");
				cl = d->waitForMouseClick();
                                //refreshing offscreen calibration
				if (cl==1){
                                for(int i=1;i<5;i++){
                                    d->clearScreen();
                                    string s = stringify(i);
                            ...
                            d->waitForMouseClick() ;
                            string testString ;
                            testString = getARandomString(stringSize,alphabet);
                            switch( task ){
                                case 1 : testString = a_41230.at(ind1);ind1++;break ;
                                case 2 : testString = a_03214.at(ind2);ind2++;break ;
                                case 3 : testString = d_41230.at(ind3);ind3++;break ;
                                case 4 : testString = d_03214.at(ind4);ind4++;break ;
                            }
				cout<<r<<"-"<<task<<endl;
                            d->clearScreen() ;
                            d->displayFixationBlink();
                            if(mode==0){
                             imst= "audioEvent start STIMULUS PLAYING "+stringify(task)+"_"+stringify(r)+"_"+testString;
                             d->pushEvent(imst);
                                       for(int i = 0 ; i < (int)testString.size(); i++){
                                            if( Mix_PlayMusic( audio_map[charmap[testString.substr(i,1)]], 0 ) == -1 ) { return 1; }
                                            while( Mix_PlayingMusic() == 1 ){}
                                            d->waitFrames(onsetDel);
                                            }
                             imst= "audioEvent stop STIMULUS PLAYING "+stringify(task)+"_"+stringify(r)+"_"+testString;
                             d->pushEvent(imst);
                             }else{
                             if(task== 0){
                               imst= "displayEvent start DUMMY ONSET "+stringify(task)+"_"+stringify(r)+"_"+testString;
                             }else{
                               imst= "displayEvent start STIMULUS ONSET "+stringify(task)+"_"+stringify(r)+"_"+testString;
                             }  
                             ...
                             switch( mode ){
                                    ...
                                    case 3 : displayLinear(testString,onsetDel,white_sapece_distance) ; break ;
                              ...
                              if(task== 0){
                                imst= "displayEvent stop DUMMY ONSET "+stringify(task)+"_"+stringify(r)+"_"+testString;
                              }else{
                                imst= "displayEvent stop STIMULUS ONSET "+stringify(task)+"_"+stringify(r)+"_"+testString;
                              }  
                           ...
                           if(cue_wait_frames != 0) d->waitFrames(cue_wait_frames) ;
                            d->displayFixation() ;
                                ...
                                if(task == 0 ) {d->displayRedDotFixation();d->waitFrames(10);}
                                if(task == 1 || task ==2 ){if( Mix_PlayMusic( recallCueMusic, 0 ) == -1 ) { return 1; }
                                    while( Mix_PlayingMusic() == 1 ){}
                                }
                                if(task==3 || task==4){
                            ...
                            if(task!=0){
                                ...
                                if((task==1 || task==2) && answer.compare(ascSort(testString))==0){
                                ...
                                if((task==3 || task==4) && answer.compare(desSort(testString))==0){
                                    d->pushEvent("answer was correct");
                                    correctDes++;
                                    d->displayText(":)");
                                    d->waitFrames(20);
            ...
            d->pushEvent("number of correct ascending sorting :"+stringify(correctAsc));
            d->pushEvent("number of incorrect ascending sorting :"+stringify(incorrectAsc));
            d->pushEvent("number of correct descending sorting :"+stringify(correctDes));
            d->pushEvent("number of incorrect descending sorting :"+stringify(incorrectDes));
            d->clearScreen();
