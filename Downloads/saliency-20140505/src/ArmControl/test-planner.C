/*! @file ArmControl/test-planner.C  grab slient objects */
...
// $HeadURL: svn://isvn.usc.edu/software/invt/trunk/saliency/src/ArmControl/test-planner.C $
// $Id: test-planner.C 10794 2009-02-08 06:21:09Z itti $
//
...
//  armSimGUI->addImage(armControllerArmSim->getPIDImagePtr());
  ...
  while(1)
  {
    ...
    if (clickLoc.isValid())
    {
     ...
     LINFO("MOve beefore");
     //armPlanner->move(desireObject,0.01);
     LINFO("MOve AAfter");
    ...
    //LINFO("Eye Pan %f Tilt %f",beoHeadBrain->getRightEyePanPos(),beoHeadBrain->getRightEyeTiltPos());
