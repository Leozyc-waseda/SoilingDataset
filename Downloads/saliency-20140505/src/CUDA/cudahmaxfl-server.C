/*!@file CUDA/cudahmaxfl-server.C CUDA Hmax Label Server class */
...
// $HeadURL: svn://isvn.usc.edu/software/invt/trunk/saliency/src/CUDA/cudahmaxfl-server.C $
// $Id: cudahmaxfl-server.C 13497 2010-05-28 00:37:41Z dparks $
//
...
int main(const int argc, const char **argv)
{
  ...
  if (mgr->parseCommandLine(
                            (const int)argc, (const char**)argv, "<cudadev> <labelFile> <c1patchesDir> <featuresFile> <localport> <server_ip> <serverport> <svmModelFile> <svmRangeFile> ", 7, 9) == false)
    return 1;
  ...
  c1PatchesBaseDir = mgr->getExtraArg(2);
  featuresFileName = mgr->getExtraArg(3);
  localPortStr = mgr->getExtraArg(4);
  serverIP = mgr->getExtraArg(5);
  serverPortStr = mgr->getExtraArg(6);
  // If we are given the SVM info, load it in
  if(mgr->numExtraArgs() > 7)
  {
