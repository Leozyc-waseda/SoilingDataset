/*!@file CUDA/runcudacbcl.C Test CUDA Hmax CBCL Model */
...
// $HeadURL: svn://isvn.usc.edu/software/invt/trunk/saliency/src/CUDA/runcudacbcl.C $
// $Id: runcudacbcl.C 13658 2010-07-13 23:43:20Z lior $
//
...
int main(const int argc, const char **argv)
{
  ...
  if (mgr->parseCommandLine(
                            (const int)argc, (const char**)argv, "<cudadev> <c01patches> <c1patches> <dir|list:images> <id> <outputfile>", 6, 6) == false)
    return 1;
  ...
  idArg = mgr->getExtraArg(4);
  c2FileName = mgr->getExtraArg(5);
  ...
  for(unsigned int imgInd=0;imgInd<imageNames.size();imgInd++){
    ...
    // Really dumb hack to make sure CBCL code doesn't break
    inputf = rescaleBilinear(inputf,256,256);
    ...
    hmax.writeOutFeatures(c2FileName,id,clearFile);
    clearFile = false;
