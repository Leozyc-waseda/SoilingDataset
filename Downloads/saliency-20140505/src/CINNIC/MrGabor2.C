/*!@file CINNIC/MrGabor2.C This file is specifically designed to place gabors
  in line with polat and sagi 1994 and should not be used for generic
  gabor placement unless you understand this file -The Management */

// $HeadURL: svn://isvn.usc.edu/software/invt/trunk/saliency/src/CINNIC/MrGabor2.C $
// $Id: MrGabor2.C 9412 2008-03-10 23:10:15Z farhan $
...
int main(int argc, char* argv[])
{
  ...
  intermedImages.resize(gaborNumber,giTemp);
  ...
  for(int i = 0; i < gaborNumber; i++)
  {
    ...
    intermedImages[i].resize(sizeX,sizeY,ZEROS);
    //Raster::VisuFloat((theGaborImages[i]+128), 0, sformat("%s.gaborI%d.pgm",imageSaveName,i));
    LINFO("Pasting image %d at %d,%d with b %f",i,xx,yy,backGround);
    if(i == 0)
    {
      theGaborImages[0] += 128;
      intermedImages[0] += 128;

    }
    inplacePaste(intermedImages[i], theGaborImages[i],
                                   Point2D<int>(xx,yy));
    //Raster::VisuFloat(intermedImages[i], 0, sformat("%s.gaborC%d.pgm",imageSaveName,i));

  }

  finalImage = intermedImages[0];

  for(int i = 1; i < gaborNumber; i++)
  {
    //add  mask gabor 1 from mask gabor2
    finalImage += intermedImages[i];
  }


  //Raster::VisuFloat(finalImage, 0, imageSaveName, RASFMT_PNM);
  convert = finalImage;
  Raster::WriteGray(convert,sformat("%s/%s.pgm",saveDir,imageSaveName));
  //Raster::VisuFloat(masterImage, 0, sformat("%s.master.pgm",imageSaveName));
  convert = masterImage;
  Raster::WriteGray(convert,sformat("%s/%s.master.pgm",saveDir,imageSaveName));
  (void) time(&t2);
