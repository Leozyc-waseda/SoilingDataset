...
template<int dw> void GaussLowPass(SImage<float> &limg, SImage<float> &oimg,
                                   float variance);
...
template <class T> void DericheLowPass(SImage<T> &src, SImage<T> &dst,
...
template <class S, class T> void RGBToGrey(SImage<S> &src, SImage<T> &dst);
/// Convert from UYVY (two value per pixel) to grey-level image
/** @param src UYVY source image
    @retval dst grey-level image */
template <class S, class T> void YUVToGrey(SImage<S> &src, SImage<T> &dst);
/// Convert from RGB to YUV
/** @param src RGB source image
    @retval yimg luminance component image
    @retval uimg u colour component image
    @retval vimg v colour component image */
template <class S, class T> void RGBToYUV(SImage<S> &src, SImage<T> &yimg,
                                          SImage<T> &uimg, SImage<T> &vimg);
...
template <class S, class T> void YUVToRGB(SImage<T> &yimg, SImage<T> &uimg,
                                          SImage<T> &vimg, SImage<S> &dst);

/// Point-wise multiply two images
/** @param src source image
    @retval dst product image */
template <class T> void operator*=(SImage<T> &dst, SImage<T> &src);
...
template <class T> void Laplace(SImage<T> &src, SImage<T> &dst);
/// Compute absolute difference between two images
/** @param src source image
    @retval dst absolute difference image */
template <class T> void AbsDiff(SImage<T> &src, SImage<T> &dst);
/// Compute absolute value of image
/** @param src source image
    @retval dst absolute value image */
template <class T> void Abs(SImage<T> &src, SImage<T> &dst);
...
template <class T> void RotatingSum(SImage<T> &src, SImage<T> &dst,
                                    int dw, int dh);

/// Rescale image values
/** @param scale rescaling factor
    @retval img image to be rescales */
template <class T> void ReScale(SImage<T> &img, float scale);
/// Copy an data array to image
/** @param indat array of image data
    @retval img destination image */
template <class T, class S> void Copy(T* indat, SImage<S> &img);
/// Copy data from one image to another
/** @param src source image
    @retval dst destination image */
template <class T, class S> void Copy(SImage<T> &src, SImage<S> &dst);
...
template <class T> void LowPass(SImage<T> &img, SImage<T> &out);
/// Low-pass an image with variance \f$\sigma^2\f$ = 1.0
/** @param img source image (zero boundary assumed)
    @retval out blurred destination image */
template <class T> void LowPassZero(SImage<T> &img, SImage<T> &out);
/// Low-pass an image with variance \f$\sigma^2\f$ = 0.5
/** @param img source image (repeated boundary assumed)
    @retval out blurred destination image */
template <class T> void LowPass3(SImage<T> &img, SImage<T> &out);

/// Low-pass an image x-wise with variance \f$\sigma^2\f$ = 1.0
/** @param img source image
    @retval out blurred destination image */
template <class T> void LowPassX(SImage<T> &img, SImage<T> &out);
/// Low-pass an image y-wise with variance \f$\sigma^2\f$ = 1.0
/** @param img source image
    @retval out blurred destination image */
template <class T> void LowPassY(SImage<T> &img, SImage<T> &out);
/// Low-pass an image x-wise with variance \f$\sigma^2\f$ = 0.5
/** @param img source image
    @retval out blurred destination image */
template <class T> void LowPassX3(SImage<T> &img, SImage<T> &out);
/// Low-pass an image y-wise with variance \f$\sigma^2\f$ = 0.5
/** @param img source image
    @retval out blurred destination image */
template <class T> void LowPassY3(SImage<T> &img, SImage<T> &out);
/// Compute x-wise derivative
/** @param img source image
    @retval out x-wise derivative */
template <class T> void HighPassX3(SImage<T> &img, SImage<T> &out);
/// Compute y-wise derivative
/** @param img source image
    @retval out y-wise derivative */
template <class T> void HighPassY3(SImage<T> &img, SImage<T> &out);
...
template <class T, class S> void SubRectify(SImage<T> &img, SImage<S> &out,
...
template <class T, class S> void Rectify(SImage<T> &img, SImage<S> &out,
                                         float angle, float focal, float xshift = 0.0, float yshift = 0.0);
/// Correct an image for radial distortion
template <class T> void RadialCorrect(SImage<T> &img, SImage<T> &out,
                                      float factor);

typedef double (*DoubleFunc1)(double);
typedef double (*DoubleFunc2)(double, double);
template <class T> void SImageFunc(DoubleFunc1, SImage<T> &img, SImage<T> &out);
template <class T> void SImageFunc(DoubleFunc2, SImage<T> &img1, SImage<T> &img2,
                                  SImage<T> &out);
...
void operator+=(SImage<UCHAR> &dst, SImage<UCHAR> &src);
template<> void LowPassX<UCHAR>(SImage<UCHAR> &img, SImage<UCHAR> &out);
template<> void LowPassY<UCHAR>(SImage<UCHAR> &img, SImage<UCHAR> &out);
template<> void LowPass<float>(SImage<float> &img, SImage<float> &out);
/// @endcond
...
template <class T>
void DericheLowPass(SImage<T> &src, SImage<T> &dst, double sigma,
                    bool zeroboard = false)
{
  ...
  int w = src.GetWidth();
  int h = src.GetHeight();
  SImage<float> tmp(w, 2);
  SImage<float> buf(w, 2);
  T *srcd = src.GetData();
...
template <class T>
void DericheLowPassOld(SImage<T> &src, SImage<T> &dst, double sigma,
                       bool zeroboard = false)
{
  T *srcd = src.GetData();
  T *dstd = dst.GetData();
...
template <class T>
void SubCopy(SImage<T> &src, SImage<T> &dst, int xp, int yp)
{
  int sw = src.GetWidth();
  int sh = src.GetHeight();
  int dw = dst.GetWidth();
...
template <class T, class S>
  void SubRectify(SImage<T> &img, SImage<S> &out, float angle, float focal,
...
template <class T, class S>
  void Rectify(SImage<T> &img, SImage<S> &out, float angle, float focal,
...
template <class T>
void SImageFunc(DoubleFunc1 func, SImage<T> &img, SImage<T> &out)
{
  T *imgd = img.GetData();
  T *outd = out.GetData();
  int w = img.GetWidth();
...
template <class T>
void SImageFunc(DoubleFunc2 func, SImage<T> &img1, SImage<T> &img2, SImage<T> &out)
{
  T *im1d = img1.GetData();
  T *im2d = img2.GetData();
  T *outd = out.GetData();
