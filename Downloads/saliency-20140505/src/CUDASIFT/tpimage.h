...
class SImage {
 ...
 public:
  ...
  ~SImage() { if (localalloc) delete [] img; }
  /// Set new image size
  /** @param w image width
      @param h image height */
...
template<class T>
SImage<T>::SImage() : SImage(32, 32, NULL)
{

}

template<class T>
SImage<T>::SImage(int w, int h, T *ptr) : width(w), height(h)
{
...
template<class T>
void SImage<T>::SetSize(int w, int h)
{
  if (w==width && h==height)
    return;
  if (localalloc)
...
template<class T>
bool SImage<T>::Load(const char *filename)
{
...
template<class T>
bool SImage<T>::LoadRGB(const char *filename)
{
  std::ifstream imagefile;
...
template<class T>
void SImage<T>::Store(const char *filename, bool type, bool ascii) const
{
...
template<class T>
void SImage<T>::StoreRGB(const char *filename) const
{
  assert(!(width%3));
  std::ofstream imagefile;
  imagefile.open(filename);
...
template<class T>
void SImage<T>::StoreYUV(const char *filename) const
{
  assert(!(width%2));
  std::ofstream imagefile;
  imagefile.open(filename);
...
template <class T>
void SImage<T>::operator=(SImage<T> &src)
{
