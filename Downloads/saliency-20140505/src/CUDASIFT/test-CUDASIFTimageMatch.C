...
// $HeadURL: svn://isvn.usc.edu/software/invt/trunk/saliency/src/CUDASIFT/test-CUDASIFTimageMatch.C $
// $Id: test-CUDASIFTimageMatch.C 12962 2010-03-06 02:13:53Z irock $
//
...
int main(const int argc, const char **argv)
{
  ...
  rutz::shared_ptr<VisualObject> vo1(new CUDAVisualObject(argv[2], "", colim1));
  rutz::shared_ptr<VisualObject> vo2(new CUDAVisualObject(argv[3], "", colim2));
#else
  // create visual objects:
  ...
  //VisualObjectMatch match(vo1, vo2, voma); //Threshold defaults to 7U
  VisualObjectMatch match(vo1, vo2, voma, 8U);

  LINFO("Found %u matches between %s and %s", match.size(), argv[2], argv[3]);

  // let's prune the matches:
  uint np = match.prune(400,300);
