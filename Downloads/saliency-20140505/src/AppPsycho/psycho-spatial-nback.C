/*!@file AppPsycho/psycho-spatial-nback.C Psychophysics test to measure the effect of executive tasks on spatial memory */
...
ModelManager manager("Psycho-Spatial-nback");
...
vector<int> getA2RepeatingSequence(int numOfElements , int length){
  ...
  int repeatPlace = 2+ pickOne(length-2);
...
vector<int> getA1RepeatingSequence(int numOfElements , int length){
  vector<int> nonrepeatingsequence = getA1NonRepeatingSeqence(numOfElements,length);
  int repeatPlace = 1+ pickOne(length-1);
