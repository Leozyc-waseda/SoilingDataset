...
// $HeadURL: svn://isvn.usc.edu/software/invt/trunk/saliency/src/AppPsycho/psychoWM.C $
// $Id: psychoWM.C 12962 2010-03-06 02:13:53Z irock $
//
...
//                             d->displayText(testString,Point2D<int>(x,y),PixRGB<byte>(0,0,0),PixRGB<byte>(128,128,128),true) ;
                            d->waitFrames(onsetDel) ;
                            d->clearScreen() ;
                            if(argMap["mask"].compare("y")==0) showMask(mask_onset_frames,argMap["alphabet"]);
                            if(cue_wait_frames != 0) d->waitFrames(cue_wait_frames) ;
