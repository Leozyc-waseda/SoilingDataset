/*!@file AppPsycho/psycho-calibtest.C movement field test */
...
// $HeadURL: svn://isvn.usc.edu/software/invt/trunk/saliency/src/AppPsycho/psycho-calibtest.C $
...
extern "C" int main(const int argc, char** argv)
{
  ...
  ModelManager manager("Psycho MovementField");
  ...
  d->displayText("<SPACE> to start the movement field");
  int c = d->waitForKey();
  if (c == ' ') {

      d->displayEyeTrackerCalibration(6,6);
      d->clearScreen();
      d->displayText("Half done");
      c = d->waitForKey();
      d->clearScreen();
      d->displayEyeTrackerCalibration(6,6);
  }
