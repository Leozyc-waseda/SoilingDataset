/*!@file AppPsycho/psycho-vmc-r-pt.C  stimulus presentation program which presents items of visual working memory randomly on the screen*/ 
...
// $HeadURL: svn://isvn.usc.edu/software/invt/trunk/saliency/src/Psycho/psycho-vmc-r-pt.C$
...
std::string getUsageComment()
{
  ...
  com += "\nmin-radius=[>=0] (minimum radius - in pixel - of center of patches) {default=0}\n";
  com += "\nmax-radius=[>0] (maximum radius - in pixel - of center of patches) {default=300}\n";
 ...
 // com += "\nwait-time=[>0] (waiting for response in milliseconds right after starting the tone){default=1000000} \n" ;
  ...
  com += "\nmode=[0,1 or 2](0 for just responding to the first tone, 1 for parity check on the first tone and 2 for parity check on two signals)\n";
  return com ;
}
...
extern "C" int main ( const int argc, char** argv )
  {
   ...
   // argMap["delay-frames"] = "120";
    ...
    argMap["min-radius"] = "0";
    argMap["max-radius"] = "300" ;
    argMap["num-of-signals"]="5";
    argMap["mode"]="0";
    argMap["subject"]="";
    //argMap["wait-time"] = "1000000";
    ...
    int minRadius = atoi(argMap["min-radius"].c_str());
    int maxRadius = atoi(argMap["max-radius"].c_str());
	...
	centers = getARandomArrayOfVSCenters(numOfItems,minRadius,maxRadius,distanceBetweenPatches,Point2D<int> ((d->getWidth()-patchsize)/2,(d->getHeight()-patchsize)/2 ));
	...
	d->clearScreen(); 
	d->displayFixation();
	//performParityTestingTask( d ,mode, numOfSignals, waitTime ,  toneSequence , responseSequence);
