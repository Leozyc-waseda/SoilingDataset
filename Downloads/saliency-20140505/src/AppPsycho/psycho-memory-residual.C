/*!@file AppPsycho/psycho-memory-residual.C Psychophysics test to measure residual memory for different task */
...
ModelManager manager("Psycho-Memory-Residual");
...
int spatial_memory_retrival(vector<int> angles ,int rad = 100 ){
	...
	d->waitFrames(20);
	d->clearScreen();
	string ans = getDigitSequenceFromSubject("yn");
	if(flip==0){
		res = 0 ;
...
std::string getUsageComment(){
        ...
        com += "\nlogfile=[logfilename.psy] {default = psycho-mem-res.psy}\n" ;
...
extern "C" int main(const int argc, char** argv)
{
        ...
        argMap["experiment"]="memory-residual-test";
        argMap["logfile"]="psycho-mem-res.psy" ;
  ...
  // let's display an ISCAN calibration grid:
        ...
        //let's see in what mode the user like to run the program
            ...
            int isi = atoi(argMap["isi"].c_str()) ;
	    int task = atoi(argMap["task"].c_str());
		...
		string ans = getDigitSequenceFromSubject(masterString);
