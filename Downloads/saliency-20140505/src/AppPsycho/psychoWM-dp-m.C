...
// $HeadURL: svn://isvn.usc.edu/software/invt/trunk/saliency/src/AppPsycho/psychoWM-dp-m.C $
// $Id: psychoWM-dp-m.C 12962 2010-03-06 02:13:53Z irock $
//
...
extern "C" int main(const int argc, char** argv)
{
        ...
        argMap["cue-type"] = "v" ;
        argMap["sound-dir"]="..";
        argMap["min-wait-frames"] = "150" ;
        argMap["max-wait-frames"] = "240" ;
  ...
  // let's display an ISCAN calibration grid:
        ...
        //let's see in what mode the user like to run the program
            ...
            int md = atoi(argMap["min-wait-frames"].c_str());
            int Md = atoi(argMap["max-wait-frames"].c_str());
            ...
            while( cr <numOfTests ){
                            ...
                            std::string imst = "===== Showing image: tst"+ stringify(cr)+ ".png =====";
                            d->pushEvent(imst);
                            eyet->track(true);
                            ...
                            imst = "===== Showing image: def"+ stringify(cr)+ ".png =====";
                            d->pushEvent(imst);
                            eyet->track(true);
                            d->waitFrames(cue_onset_frames);
                            long st = d->getTimerValue() ;//we take this time as the start time of the operation, right after the signal for sorting
                            d->clearScreen() ;
                            d->pushEvent("second recording starts");
                            d->pushEvent("the sequence for memorization is : "+testString) ;
                            d->waitFrames(rand()%(Md-md) + md) ;
                            long et = d->getTimerValue();//we take this time as the end time of the operation, right after recieving the mouse click
                            eyet->track(false);
                            d->pushEvent("task ends") ;
                            d->pushEvent("second part time:"+stringify(et-st));
                            allTiming.push_back(et-st);
                            //let's register subject's answer
                            string  answer = getDigitSequenceFromSubject(argMap["alphabet"] , testString.size());
                            d->pushEvent("subject keyed : "+answer);
                            //and let's see if the suject was correct

                            if(answer.compare(testString)==0){
                                    d->pushEvent("answer was correct");
                                    correctAnswersTiming.push_back(et-st) ;
                            }else{
                                    d->pushEvent("answer was incorrect");
                                    incorrectAnswersTiming.push_back(et-st);
                            }


            }
            d->pushEvent("total number of memory trials :"+stringify(correctMemory+incorrectMemory)) ;
            d->pushEvent("number of correct memory recall : "+ stringify(correctMemory)) ;
            d->pushEvent("total number of trials :"+ stringify(allTiming.size())) ;
            d->pushEvent("number of correct answers :" + stringify(correctAnswersTiming.size()));
            d->pushEvent("number of incorrect answers :"+ stringify(incorrectAnswersTiming.size())) ;
