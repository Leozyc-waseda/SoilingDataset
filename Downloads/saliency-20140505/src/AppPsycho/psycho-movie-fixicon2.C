...
static bool cacheFrame(nub::soft_ref<InputMPEGStream>& mp,
                       std::deque<VideoFrame>& cache, const bool flip)
{
...
static int submain(const int argc, char** argv)
{
  ...
  uint nbmovies = manager.numExtraArgs();
	uint nbmovses = 5; //number of movies per session
	int index[nbmovies], sindex[nbmovses];
        index[0]=0; // for demo...
	// only randomize order in its own session
        // this code does not work if total number of movies not multiple of movies per session
	LINFO("Randomizing movies...");
	int iidx = 0;
	for (uint i=0; i<nbmovies; i++) {
		sindex[i%nbmovses] = i;
		if ((i%nbmovses) == (nbmovses-1)){
			randShuffle(sindex, nbmovses);
			for (uint j=0; j<nbmovses; j++) {
				index[iidx] = sindex[j];
				iidx++;
			}
		}
  ...
  for (uint i = 0; i < nbmovies; i ++)
    {
      ...
      LINFO("'%s' ready.", manager.getExtraArg(index[i]).c_str());
			LINFO("Video loaded (%i/%i).  Press [Space] to continue, or [ESC] to quit the program.", i+1, nbmovies);
      ...
      d->pushEvent(sformat("-- Flipped: %d --", int(hflip.getVal())));
      ...
      et->track(false);
			...
			// take a break for every 5 clips, or do a quick drift correction
      if (i<nbmovies-1){
	      if (etc->getModelParamString("EyeTrackerType").compare("EL") == 0){
  	      d->closeDisplay();
          et->openSDL();
					if ((i+1)%5 == 0){
						//5 clips, take a break
  					d->clearScreen();
          ...
          LINFO("Switching SDL for Calibration ");
        }else{
