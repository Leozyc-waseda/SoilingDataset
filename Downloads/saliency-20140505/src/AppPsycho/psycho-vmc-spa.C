/*!@file AppPsycho/psycho-vmc-spa.C  stimulus presentation program which presents items of visual working memory randomly on the screen concurrent with spatial attenion test*/ 
...
// $HeadURL: svn://isvn.usc.edu/software/invt/trunk/saliency/src/Psycho/psycho-vmc-spa.C$
...
std::string getUsageComment()
{
  ...
  com += "\nvs-radius=[>50] (radius of the virtual circle for presenting the items){default=200}\n" ;
  ...
  com += "\nsp-presenation-frames=[>1] (number of frames for presenation of spatial-attention stimulus) {default=6}\n" ;
  return com ;
}
...
extern "C" int main ( const int argc, char** argv )
  {
    ...
    argMap["vs-radius"]="200";
    argMap["sp-presenation-frames"] = "6" ;
    ...
    int vs_radius = atoi(argMap["vs-radius"].c_str());
    ...
    int sp_frames = atoi(argMap["sp-presenation-frames"].c_str());
	...
	for(int cs = 0 ; cs < (int)(changeSizes.size());cs++){
	  for(int c = 0 ; c <2 ; c++){
	    for(int t = 0 ; t < numOfIndividualTrials ; t++){  
	      vector<int> tv(3,0);
	      tv[0] = arraySizes.at(s);
	      tv[1] = changeSizes.at(cs);
	      tv[2] =  c ;
    ...
    int numOfTests = numOfIndividualTrials* (int)(arraySizes.size())*(int)(changeSizes.size());
	...
	int changeFlag = trialsVector.at(cr).at(2);
	groundTruth.push_back(changeFlag);
	int numOfItems = trialsVector.at(cr).at(0) ;
	int numOfTargetChanges = trialsVector.at(cr).at(1) ;
	vector< Point2D<int> > centers ;
	centers = getACircularArrayOfVSCenters(numOfItems,vs_radius,0,Point2D<int> ((d->getWidth()-patchsize)/2,(d->getHeight()-patchsize)/2 ));
	
	vector<int> colors = getRandomNonRepeatingNumbers(numOfItems,myColorMap.size());
	vector<int> newColors = colors;
	vector<int> changedPositions ;
	if(changeFlag == 1) newColors = repalceColors(colors,myColorMap.size(),numOfTargetChanges,changedPositions);
	...
	d->pushEvent("number of target change: " + stringify<int>(numOfTargetChanges));
	
	
	//this is going to be the number of frames that we will wait to show target-distractor screen
	int frameWaits = rand()% delay/4 + delay/4;
	//this is going to be the time we will wait after presenting target-distrator screen for the response
	long leftMilliseconds = (delay - frameWaits - sp_frames -1 )*16666;
	int showTarget = rand()%2 ;
	int targetPresentationLocation=-1;
	if(showTarget == 1) targetPresentationLocation = rand()%centers.size();
	if(itIsInThere(targetPresentationLocation,changedPositions)){
	  d->pushEvent("target is persented @ the location of change");
	}else{
	  if(targetPresentationLocation !=-1){
	    d->pushEvent("target is presented @ a non-change location");
	  }
	}
	//**here sequence of stimulus presenation starts
	d->displayFixationBlink(-1,-1,3,1);
	d->clearScreen();
	displayVMStimulus( d,myColorMap , colors ,  centers, patchsize,"circle");
	d->waitFrames(stimulusPresentationDuration*numOfItems);
	d->clearScreen(); 
	d->displayFixation();
	d->waitFrames(frameWaits);
	if(showTarget==1) {
	  diplayTargetDistractors(d,"o","x",centers,targetPresentationLocation);
	}else{
	  diplayTargetDistractors(d,"o","x",centers,-1);
	}
	d->waitFrames(sp_frames);
	d->clearScreen();
	bool clickFlag = listenForClicks(d,leftMilliseconds,1);
	
	d->displayFixationBlink(-1,-1,3,1); 
	displayVMStimulus( d, myColorMap, newColors ,  centers, patchsize,"circle");
	...
	if(clickFlag){
	  if( showTarget == 1){
	    d->pushEvent("identified the target correctly by right click");
	  }else{
	    d->pushEvent("identified the target incorrectly by right click");
	  }
	}else{
	   if( showTarget == 1){
	    d->pushEvent("missed the target");
	  }else{
	    d->pushEvent("target was missing and did not respond correctly");
	  }
	}
    ...
    titles.push_back("trial-number:"); titles.push_back("array-size");titles.push_back("change-size"); titles.push_back("change");titles.push_back("answer");
