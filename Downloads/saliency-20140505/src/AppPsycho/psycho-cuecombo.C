...
#define NTRANSFER 5 /* number of transfer blocks */
...
static int submain(const int argc, char** argv)
{
  ...
  // main loop:
  // prepare two angles for left and right association
  ...
  for (int block = 0; block < NBLOCKS; block ++) {
    ...
    while(!stair.done()) {
      ...
      double categoryVal = 0.5 + (flip ? -0.5 : 0.5) * stair.generateStim();
      ...
      // get angle that matches with 100% validity to identity
      double selAngle = (flip) ? leftAngle : rightAngle;
