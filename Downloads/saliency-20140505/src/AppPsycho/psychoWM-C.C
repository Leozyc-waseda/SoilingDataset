...
// $HeadURL: svn://isvn.usc.edu/software/invt/trunk/saliency/src/AppPsycho/psychoWM-C.C $
// $Id: psychoWM-C.C 12962 2010-03-06 02:13:53Z irock $
//
...
vector<long> getInterrunptVector(long minIntVal=500000 , long maxIntVal=2000000 , uint rep=10){
        vector<long> tempVector;
        vector<long> intVector;
        for (uint i = 0 ;  i < rep ; i++){
                while( tempVector.size() < i+1 ){
                        long tl = random()%(rep*maxIntVal) ;
                        bool flag = true ;
                        for(uint j = 0 ; j < tempVector.size() ; j++ ){
                                if(abs(tl-tempVector[j]) < minIntVal) flag = false ;
                        }
                        if(flag) tempVector.push_back(tl) ;
                }
        }
        std::sort(tempVector.begin(), tempVector.end());
        long in = 0 ;
        for( uint i = 0 ; i < tempVector.size() ; i++ ){
                intVector.push_back(tempVector[i] - in) ;
                in = tempVector[i] ;
        }
        intVector.push_back(922036854797);//this is a the last interrupt which is set to a big number
        return intVector ;
}
...
extern "C" int main(const int argc, char** argv)
{
        ...
        argMap["experiment"]="sorting_strings_in_working_memory-with-color-presentation";
        argMap["logfile"]="psycho-wm-cc.psy" ;
        ...
        //let's count the rounds
            ...
            while( cr <numOfTests ){
                    ...
                    if(mP > memPrb){
                            ...
                            int cs = 0 ;
                            d->displaySDLSurfacePatch(surfVector[cs] , &offs , NULL , -2 , false ,true ) ;
                            long dst = 0 ;
                            long det = 0 ;
                            long dl = 0 ;
                            ...
                            int wrongclick = 0 ;
                            vector<long> interruptsVector = getInterrunptVector(ilb,iub,500);
                            int ind = 0 ;
                            dl = interruptsVector[ind] ;ind++ ;
                             while( !exitFlag ){
                            ...
                            if(missed <= max_miss && getAvarage(reactionTimes)<= min_reaction_time && wrongclick <= max_wrong){
                                        ...
                                        if(af){
                                                ...
                                                d->pushEvent("answer was incorrect");fssr++;
                                        }
                                }else{
