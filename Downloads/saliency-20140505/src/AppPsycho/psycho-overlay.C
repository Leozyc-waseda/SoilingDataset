/*!@file AppPsycho/psycho-overlay.C Psychophysics display of movies */
...
// $HeadURL: svn://isvn.usc.edu/software/invt/trunk/saliency/src/AppPsycho/psycho-overlay.C $
// $Id: psycho-overlay.C 13712 2010-07-28 21:00:40Z itti $
//
...
static int submain(const int argc, char** argv)
{
  ...
  if (manager.parseCommandLine(argc, argv,
                               "<movie1.mpg> <overlay_image1>..."
                               "<movieN.mpg> <overlay_imageN>", 1, -1)==false)
    return(1);
  ...
  if (manager.numExtraArgs() % 2 != 0)
    LFATAL("You need to have an overlay image for every movie!");

  uint nbmovies = manager.numExtraArgs() / 2;
  int index[nbmovies];
  int imgindex[nbmovies];
  ...
  if (c == ' ')
    {
      LINFO("Randomizing movies and images...");
      randShuffle(index,nbmovies);
      randShuffle(imgindex,nbmovies);
    }
  ...
  for (uint i = 0; i < nbmovies; i ++)
    {
      ...
      //get our image to be overlayed
      LINFO("Loading %s", manager.getExtraArg(imgindex[i]*2+1).c_str());
      Image<PixRGB<byte> > cimage =
        Raster::ReadRGB(manager.getExtraArg(imgindex[i]*2+1));
      //transparent pixel should be in upper left hand corner
      PixRGB<byte> currTransPixel = cimage[0];

      // cache initial movie frames:
      bool streaming = true;
      LINFO("Buffering '%s'...", manager.getExtraArg(index[i]*2).c_str());
      mp->setFileName(manager.getExtraArg(index[i]*2));
      ...
      LINFO("'%s' ready.", manager.getExtraArg(index[i]*2).c_str());

      // give a chance to other processes (useful on single-CPU machines):
      sleep(1); if (system("/bin/sync")) LFATAL("sync failed");
