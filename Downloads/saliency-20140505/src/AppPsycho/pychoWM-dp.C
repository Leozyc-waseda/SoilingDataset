...
// $HeadURL: svn://isvn.usc.edu/software/invt/trunk/saliency/src/AppPsycho/pychoWM-dp.C $
// $Id: pychoWM-dp.C 12962 2010-03-06 02:13:53Z irock $
//
        ...
        //let's see in what mode the user like to run the program
            ...
            while( cr <numOfTests ){
                    ...
                    if(mP > memPrb){
                            ...
                            d->pushEvent("manipulation time:"+stringify(et-st));
                            ...
                            if(argMap["mask"].compare("y")==0) showMask(mask_onset_frames,argMap["alphabet"]);
                            std::string imst = "===== Showing image: otst"+ stringify(other)+ ".png =====";
                            d->pushEvent(imst);
                            eyet->track(true);
                            if(cue_wait_frames != 0) d->waitFrames(cue_wait_frames) ;
                            eyet->track(false);
                            if(argMap["cue-type"].compare("v")==0){
                                    d->displayFixationBlink(d->getWidth()/2,d->getHeight()/2,1,cue_onset_frames);
                            }else{
                                    if( Mix_PlayMusic( recallCueMusic, 0 ) == -1 ) { return 1; }
                                    while( Mix_PlayingMusic() == 1 ){}
                            }
