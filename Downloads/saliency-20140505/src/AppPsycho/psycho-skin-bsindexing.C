...
/////////////////////////////////////////
void showDynamicSlide(Matrix& theMap , map<string,SDL_Surface*>& cmap, int sT ){
...
/////////////////////////////////////////
void showStaticSlide(Matrix& theMap , map<string,SDL_Surface*>& cmap, int sT ){
        d->pushEvent("exposure time will be "+stringify(sT));
        int cs = theMap.getNumOfColumns();
        int rs = theMap.getNumOfRows() ;
        map<string , int> clipNumMap;
        int imageWidth = -1 ;
...
int readBlock(ifstream& inFile){
        ...
        while( inFile.getline(ch , 1000) ){
                ...
                if(line.substr(0,16).compare("initial_exposure")==0){
                        string::size_type position = line.find("=");
                        char rf[10] ;
                        strcpy(rf,line.substr(position+1).c_str());
                        ie = atoi(rf);
                }
                if(line.substr(0,8).compare("skip_exp")==0){
                        string::size_type position = line.find("=");
                        char rf[10] ;
                        strcpy(rf,line.substr(position+1).c_str());
                        skip_by = atoi(rf);
                }
                if(line.substr(0,10).compare("block_size")==0){
                        string::size_type position = line.find("=");
                        char rf[10] ;
                        strcpy(rf,line.substr(position+1).c_str());
                        block_size = atoi(rf);
                }
                if( line.substr(0,15).compare("max_sensitivity")==0 ){
                        string::size_type position = line.find("=");
                        char rf[10] ;
                        strcpy(rf,line.substr(position + 1 ).c_str());
                        maxSen = atof(rf);
                }
                if( line.substr(0,12).compare("max_accuracy")==0 ){
                        string::size_type position = line.find("=");
                        char rf[10] ;
                        strcpy(rf,line.substr(position + 1 ).c_str());
                        maxAcc = atof(rf);
        ...
        */
        do{
                ...
                float accuracy = 0.0f ;
                int* experiment = new int[block_size] ;
                int* report = new int[block_size] ;
                ...
                for( int i = 0 ; i < block_size ; i++ ){
                        ...
                        */
                        ...
                        d->pushEvent("started showing the test image");
                        if( staticFlag ){
                                showStaticSlide(*map,*classesMap , exposureTime );
                        } else {
                                showDynamicSlide(*map,*classesMap , exposureTime );
                        }
                        d->pushEvent("ended showing the test image");
                        /*let's ask the subject if s/he identified the target*/
                        report[i] = getResponse();
                        d->clearScreen();
                }
                ...
                */
                ...
                maxSensitivity = max(sensitivity,maxSensitivity) ;
                maxAccuracy = max(accuracy,maxAccuracy);
                exposureTime += skip_by ;
                d->pushEvent("experiment srting : " + experimentString);
                d->pushEvent("report string : " + reportString);
                d->pushEvent("sensitivity : "+ stringify(sensitivity)) ;
                d->pushEvent("accuracy : " + stringify(accuracy));
        }while( maxSensitivity < maxSen || maxAccuracy < maxAcc  );
        ...
        classesMap->~map<string,SDL_Surface*>();
        rawPattenMatrix->~Matrix();
        vars->~vector<Matrix*>() ;
        messages->~vector<string>() ;
        exclusionList->~vector<string>() ;
