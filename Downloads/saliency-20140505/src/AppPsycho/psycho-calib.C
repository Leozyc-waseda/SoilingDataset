...
// $HeadURL: svn://isvn.usc.edu/software/invt/trunk/saliency/src/AppPsycho/psycho-calib.C $
...
extern "C" int main(const int argc, char** argv)
{
        ...
        argMap["experiment"]="testing-calibration";
        argMap["logfile"]="psycho-calib.psy" ;
        argMap["subject"]="" ;
        argMap["memo"]="" ;
        argMap["x-grid"] = "10";
        argMap["y-grid"] = "10";
        ...
        int y_g = atoi(argMap["y-grid"].c_str()) ;
         SDL_Rect offset ;
  ...
  // let's display an ISCAN calibration grid:
            ...
            if (cl == 1) d->displayEyeTrackerCalibration(4,5,1 , true);
            d->clearScreen();
            ...
            for(int i = 0 ; i <= x_g + 1 ; i++){
                for(int j = 0 ; j <= y_g +1 ; j++){
                        int x = i* d->getWidth()/(x_g+1);
                        int y = j* d->getHeight()/(y_g+1);
                        if( x > d->getWidth()-5) x = d->getWidth() -5 ;
                        if( y > d->getHeight()-5) y = d->getHeight() -5 ;
                        offset.x = x;
                        offset.y = y;
                        d->displaySDLSurfacePatch(mark , &offset,NULL , -2,false, true);
                        d->waitForMouseClick() ;
                        std::string imst = "===== Showing image: im"+stringify(i)+"-"+stringify(j)+".png =====";
                        d->pushEvent(imst);
                        eyet->track(true);
                        d->waitForMouseClick() ;
                        eyet->track(false);
