...
void performSpatialCueingBlock(nub::soft_ref<PsychoDisplay> d,nub::soft_ref<EyeTracker> et, int numOfTrials ,int numOfTestItems ){
  ...
  d-> displayText(" NO Dummy character "+stringify<int>(numOfTrials)+" trials.");
  d->waitFrames(360);
  d-> clearScreen();
  ...
  while(cr < numOfTrials){
    ...
    vector<int> myChars = getRandomNonRepeatingNumbers(min(numOfTestItems,(int)fourStrings.size()),fourStrings.size());
    ...
    for(int i = 0 ; i < (int)myChars.size() ; i++) chars.push_back(fourStrings.at(myChars.at(i)));//+
      ...
      if(i> 0 ){
        ...
        if(false){
          ...
          if (changeFlag == 1){
             boxIndex = boxIndexHistory.at(boxIndexHistory.size()-1);
             do{
              charIndex = rand()%min(numOfTestItems,(int)fourStrings.size()) ;
...
void performDistractedSpatialCueingBlock(nub::soft_ref<PsychoDisplay> d,nub::soft_ref<EyeTracker> et, int numOfTrials ,int numOfTestItems ){
  
  d-> displayText(" LOOK FOR DUMMY CHARACTERS IN NEXT "+stringify<int>(numOfTrials)+" trials!");
   d->waitFrames(360);
  d-> clearScreen();
  d-> displayText("press any key to start!");
  d-> waitForKey();
  ...
  while(cr < numOfTrials){
    ...
    int dummyIndex = -1 ;
    int dummyBoxIndex = -1 ;
    if (goDummy >= 1) dummyIndex = rand()%8 +1 ;
    for(int i = 0 ; i < numOfIncrements ; i++ ){
      ...
      if(i> 0 ){
        if(i == dummyIndex){
	  dummyIndex = -1 ;
          charIndex = numOfTestItems + rand()% dummyChar.size() ;
      ...
      long onsetTime = 0;
      if(charIndex>=numOfTestItems){
      charsHistory.push_back(-1*(charIndex - numOfTestItems +1));
      drawTextInTheBox( d, boxCenters.at(boxIndex), boxSize , 1,dummyChar.substr(charIndex - numOfTestItems ,1),objectPresentationFrames);
      onsetTime = d-> getTimerValue();
       d->pushEvent("Eye-Tracking starts");
      if(etFlag==1) et->track(true);
      d->waitForKey();
    ...
    if(dummyBoxIndex >= 0 ) {
      boxCounter.at(dummyBoxIndex) = 0;
      globalTrialTypeHistory.push_back(-1);
    }else{globalTrialTypeHistory.push_back(1);}
...
extern "C" int main ( const int argc, char** argv )
  {
    ...
    performDistractedSpatialCueingBlock(d,eyet,numOfTrials,numOfCountingItems);
      }else{
    performDistractedSpatialCueingBlock(d,eyet,numOfTrials,numOfCountingItems);
    performSpatialCueingBlock(d,eyet,numOfTrials,numOfCountingItems);
      }
