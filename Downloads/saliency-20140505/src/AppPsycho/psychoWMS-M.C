...
// $HeadURL: svn://isvn.usc.edu/software/invt/trunk/saliency/src/AppPsycho/psychoWMS-M.C $
// $Id: psychoWMS-M.C 12962 2010-03-06 02:13:53Z irock $
//
...
Mix_Chunk *whitenoise_chunk = NULL;
...
extern "C" int main(const int argc, char** argv)
{
        ...
        argMap["logfile"]="psycho-ws-m.psy" ;
        ...
        argMap["sound-dir"]="..";
        argMap["noise-file"]="whitenoise.wav";
        ...
        if (manager.parseCommandLine(argc, argv,
            ...
            string noisestr = argMap["sound-dir"] + "/" + argMap["noise-file"];
            whitenoise_chunk = Mix_LoadWAV(noisestr.c_str());
            if( whitenoise_chunk == NULL  )
            {
        ...
        //let's count the rounds
            ...
            if(argMap["mode"].compare("2")==0){
                    while( cr <numOfTests ){
                            ...
                            Mix_PlayChannel( -1, whitenoise_chunk , 300 ) ;
                            d->clearScreen() ;
                            ...
                            while( et - st < opDelay ){
                                    ...
                                    if (det - dst > dl ){//if we get to a point that the interrupt should be made
            ...
            if(argMap["mode"].compare("1")==0){
                    while( cr <numOfTests ){
                            ...
                            while( et - st < opDelay ){
                                    ...
                                    if (det - dst > dl ){//if we get to a point that the bloop should be made
                                            cs = getNext(cs) ;//choose the index of next speed
                                            motor->setMotor(0) ;
                                            d->waitFrames(pause_frames);
                                            motor->setMotor(speedVector[cs]) ;
            ...
            d->waitForMouseClick();
            Mix_FreeChunk( whitenoise_chunk );
            Mix_CloseAudio();
