/*!@file AppPsycho/psycho-triplecounting-spatialcue-control.C  stimulus presentation program for the Triple Counting study and 
control of spatial cueing*/ 
...
#ifndef __PSYCHO_TR_COUNT_SPCUE__
#define __PSYCHO_TR_COUNT_SPCUE__
#include <map>
...
vector<Point2D<int> > nineCenters;
int boxSize ;
int discSize ;
...
void performThreeBoxSpatialCueingBlock(nub::soft_ref<PsychoDisplay> d,nub::soft_ref<EyeTracker> et, int numOfTrials ){
  ...
  while(cr < numOfTrials){
    ...
    for(int i = 0 ; i < numOfIncrements ; ){
      ...
      drawTextInTheBox( d, boxCenters.at(boxIndex), boxSize , 1,"X",objectPresentationFrames);
...
vector<int> getLsOf(int c){
vector<int> ls;
if (c==0) {ls.push_back(5);ls.push_back(7);}
if (c==2) {ls.push_back(3);ls.push_back(7);}
if (c==6) {ls.push_back(1);ls.push_back(5);}
if (c==8) {ls.push_back(1);ls.push_back(3);}
return ls;
}


bool areBad(int a, int b ,int c){

int x = min(min(a,b),c);
int z = max(max(a,b),c);
int y = a+b+c-(x+z);

if(x+z==2*y) return true;
return false;
}

vector<int> soveTheFuckingPuzzle(vector< vector <int> > puzzle){
	vector<int> lsOf1 = getLsOf(puzzle.at(1).at(0));
	vector<int> lsOf2 = getLsOf(puzzle.at(2).at(0));
	int fSol = -1 ;
	int sSol = -1 ;
	if (areBad(lsOf1.at(0),puzzle.at(0).at(0),puzzle.at(2).at(0))){
		fSol = lsOf1.at(1);
	}else{
		if (areBad(lsOf1.at(1),puzzle.at(0).at(0),puzzle.at(2).at(0))){
			fSol = lsOf1.at(0);
		}
	}

	if (areBad(lsOf2.at(0),puzzle.at(0).at(0),puzzle.at(1).at(0))){
		sSol = lsOf2.at(1);
	}else{
		if (areBad(lsOf2.at(1),puzzle.at(0).at(0),puzzle.at(1).at(0))){
			sSol = lsOf2.at(0);
		}
	}
	...
	if(fSol == -1 ) {
		if(lsOf1.at(0) == sSol) fSol = lsOf1.at(1) ;
		if(lsOf1.at(1) == sSol) fSol = lsOf1.at(0) ;
	}
	if(sSol == -1 ) {
		if(lsOf2.at(0) == fSol) sSol = lsOf2.at(1) ;
		if(lsOf2.at(1) == fSol) sSol = lsOf2.at(0) ;
	}

	solution.push_back(fSol);solution.push_back(sSol);
	return solution;
}


void performSixBoxSpatialCueingBlock(nub::soft_ref<PsychoDisplay> d,nub::soft_ref<EyeTracker> et, int numOfTrials  ){
  PixRGB<byte> myblack ( 0,0,0 );
  PixRGB<byte> mygreen ( 0,255,0 );
  PixRGB<byte> myyellow (255,255,0);
  PixRGB<byte> myred (255,0,0);
  d-> displayText(" BOX counting "+stringify<int>(numOfTrials)+" trials in a row!");
  d-> waitForKey();
  int cr = 0 ; 
  while(cr < numOfTrials){
    vector< vector< Point2D<int> > > orderedBoxes;
    ...
    vector<int> boxCounter ;
    vector< Point2D<int> > boxCenters; 
    vector<PixRGB<byte>  > boarderColors;
    globalTrialObjectOrder.push_back(myBoxes);
    vector<int> initialCounters ;
    vector<int> lastFlash ;
    for(int i = 0 ; i < 3 ; i++ ){initialCounters.push_back(rand()%2);lastFlash.push_back(rand()%2);}
    for(int i = 0 ; i < 3 ; i++ )boxCounter.push_back(initialCounters.at(i));
    for(int i = 0 ; i < 3 ; i++){
      boxCenters.push_back(nineCenters.at(myBoxes.at(i)));
    }
    for(int i = 0 ; i < 3 ; i++ ) {
	vector< Point2D<int> > cBoxes;
        vector<int> puz ;
	cBoxes.push_back( nineCenters.at(mapCornersOn(myBoxes.at(i))));
        puz.push_back(mapCornersOn(myBoxes.at(i)));
	if(i==0) {cBoxes.push_back(nineCenters.at(4));puz.push_back(4);}
        orderedBoxes.push_back( cBoxes );
	puzzle.push_back(puz);
    }
    vector<int> solution = soveTheFuckingPuzzle(puzzle);
    for(uint i = 0 ; i < solution.size();i++){
	if(solution.at(i)!=-1) orderedBoxes.at(i+1).push_back(nineCenters.at(solution.at(i)));
    }
    
    int flp = rand()%3 ;
    if (flp == 0 ){
    boarderColors.push_back(myblack);
    boarderColors.push_back(myred);
    boarderColors.push_back(myyellow);}
    ...
    d->displayFixationBlink();
    d->displayFixation();
    drawBoxesWithColoredBorders(d, orderedBoxes, boarderColors,boxSize , 1);
    d->waitFrames(30);
    for(int i = 0 ; i < 3 ; i++){
      int j = 0 ;
      drawTextInTheBox( d, orderedBoxes.at(i).at(j), boxSize , 1,stringify<int>(initialCounters.at(i)),-1); 
      d->waitForKey();
      drawTextInTheBox( d, orderedBoxes.at(i).at(j), boxSize , 1," ",-1);
    }
    ...
    for(int i = 0 ; i < numOfIncrements ; ){
      ...
      long onsetTime = 0;
      boxCounter.at(boxIndex) = boxCounter.at(boxIndex)+1 ;    
      drawTextInTheBox( d, orderedBoxes.at(boxIndex).at(lastFlash.at(boxIndex)), boxSize , 1,"X",objectPresentationFrames);
      if(lastFlash.at(boxIndex) == 0 ) {
	lastFlash.at(boxIndex) = 1;
      } else{ lastFlash.at(boxIndex) = 0 ;}
      onsetTime = d-> getTimerValue();
      d->pushEvent("Eye-Tracking starts");
      if(etFlag==1) et->track(true);
      d->waitForKey();
    ...
    vector<char> subjectResponse;
    for(int i = 0 ; i < 3 ; i++){
      drawTextInTheBox( d, orderedBoxes.at(i).at(0), boxSize , 1,".",-1); 
      char reportedCounter = -1 ;
      char  c ;
      do{
    c = d->waitForKey();
    ...
    drawTextInTheBox( d, orderedBoxes.at(i).at(0), boxSize , 1,stringify<char>(c),-1);
    if(c == ' ' && reportedCounter != -1) break;
      }while(true);
      subjectResponse.push_back(reportedCounter);
    }
...
std::string getUsageComment()
{
  ...
  com += "\nnine-box-distance=[>0](distance between center of boxes in pixel values){defualt=100}\n";
...
extern "C" int main ( const int argc, char** argv )
  {
    ...
    argMap["logfile"]="psycho-tc-scc.psy" ;
    argMap["subject"]="";
    argMap["memo"]="";
    ...
    nineBoxDistance = atoi(argMap["nine-box-distance"].c_str());
    ...
    nineCenters.push_back(Point2D<int>(d->getWidth()/2 - nineBoxDistance , d->getHeight()/2 - nineBoxDistance));
    nineCenters.push_back(Point2D<int>(d->getWidth()/2 - nineBoxDistance , d->getHeight()/2));
    nineCenters.push_back(Point2D<int>(d->getWidth()/2 - nineBoxDistance , d->getHeight()/2 + nineBoxDistance));
    nineCenters.push_back(Point2D<int>(d->getWidth()/2  , d->getHeight()/2 - nineBoxDistance));
    nineCenters.push_back(Point2D<int>(d->getWidth()/2  , d->getHeight()/2 ));
    nineCenters.push_back(Point2D<int>(d->getWidth()/2  , d->getHeight()/2 + nineBoxDistance));
    nineCenters.push_back(Point2D<int>(d->getWidth()/2 + nineBoxDistance , d->getHeight()/2 - nineBoxDistance));
    nineCenters.push_back(Point2D<int>(d->getWidth()/2 + nineBoxDistance , d->getHeight()/2));
    nineCenters.push_back(Point2D<int>(d->getWidth()/2 + nineBoxDistance , d->getHeight()/2 + nineBoxDistance));
    ...
    for(int i = 0 ; i < numOfBlocks ; i++){
      if(mode == 0){
    performThreeBoxSpatialCueingBlock(d,eyet,numOfTrials);
    performSixBoxSpatialCueingBlock(d,eyet,numOfTrials);
      }else{
    performSixBoxSpatialCueingBlock(d,eyet,numOfTrials);
    performThreeBoxSpatialCueingBlock(d,eyet,numOfTrials);
      }
