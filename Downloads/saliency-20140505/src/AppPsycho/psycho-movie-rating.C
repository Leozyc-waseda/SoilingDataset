/*!@file AppPsycho/psycho-movie-rating.C Psychophysics display of movies */
...
// $HeadURL: svn://isvn.usc.edu/software/invt/trunk/saliency/src/AppPsycho/psycho-movie-rating.C $
// $Id: psycho-movie-rating.C 13712 2010-07-28 21:00:40Z itti $
//
...
static int submain(const int argc, char** argv)
{
  ...
  nub::soft_ref<MrawvInputStream> mp
    (new MrawvInputStream(manager, "Input Raw Video Stream",
                          "MrawvInputStream"));
  ...
  manager.setOptionValString(&OPT_EyeTrackerType, "None");
  manager.setOptionValString(&OPT_SDLdisplayDims, "1280x1024");

  // Parse command-line:
  if (manager.parseCommandLine(argc, argv,
                               "<movie1.mraw> ... <movieN.mraw>",
                               1, -1) == false)
    return(1);
  ...
  for (uint i = 0; i < nbmovies; i ++)
    {
      ...
      const int ovx = (d->getWidth() - ovw) / 2;
      const int ovy = (d->getHeight() - ovh) / 2;
      ...
      // collect subjective rating data (it will be in the psycho log):
      d->displayText("Please score this movie (1=bad .. 5=excellent)");
      do { c = d->waitForKey(); } while (c < '1' || c > '5');
      d->pushEvent(sformat("== Rating for %s = %d ==",
                           manager.getExtraArg(index[i]).c_str(), c - '0'));
