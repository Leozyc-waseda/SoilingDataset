/*!@file AppPsycho/psycho-findobjects.C Psychophysics interactive display for search in natural scenes */
...
// $HeadURL: svn://isvn.usc.edu/software/invt/trunk/saliency/src/AppPsycho/psycho-findobjects.C $
//
  ...
  while(keepgoing)
    {
      ...
      if (recording == true && recordaudio == false)
      {
        //write out audio
        char name[100];
        sprintf(name, "%s_%04d.wav", subjname.c_str(), recnb);        
        writeAudioWavFile(name, rec);
...
static const ModelOptionDef OPT_BeepOnFrame =
  { MODOPT_FLAG, "BeepOnFrame", &MOC_DISPLAY, OPTEXP_SAVE,
    "beep after each frame", "beep", '\0', "", "false" };

static const ModelOptionDef OPT_MovieFrameDelay =
{ MODOPT_ARG(float), "MovieFrameDelay", &MOC_DISPLAY, OPTEXP_SAVE,
  "delay between frames when playing movie", "movie-frame-delay", '\0', "", "33333.3333" };

static const ModelOptionDef OPT_CalibInterval =
  { MODOPT_ARG(uint), "CalibInterval", &MOC_DISPLAY, OPTEXP_SAVE,
    "clips between a break and calibration", "calib-interval", '\0', "", "3" };
...
static int submain(const int argc, char** argv)
{
  ...
  ModelManager manager("Psycho Find objects");

  OModelParam<bool> itsBeepPerFrame(&OPT_BeepOnFrame, &manager); 
  OModelParam<uint> itsCalibInterval(&OPT_CalibInterval, &manager); 
  OModelParam<float> itsMovieFrameDelay(&OPT_MovieFrameDelay, &manager); 
  OModelParam<float> itsSDLRefresh(&OPT_SDLdisplayRefreshUsec, &manager); 
  ...
  nub::soft_ref<PsychoAudio> a(new PsychoAudio(manager));
  manager.addSubComponent(a);
  ...
  std::string fname = el->getFileName();
  ...
  const bool beepon = itsBeepPerFrame.getVal();
  ...
  for (uint i = 0; i < nbmovies; i ++)
    {
      // let's do an eye-tracker calibration once in a 

      if ((i % itsCalibInterval.getVal()) == 0)
         {
           d->displayText("Take a short break. <SPACE> to start a calibration");
           d->waitForKey();
           et->calibrate(d);
           d->clearScreen();
           d->displayText("Rmember to look for cars, trucks, semis, buses, "
                          "containers, boats, planes, helicoptors, "
                          "people, and cyclists <SPACE>.");
           d->waitForKey();
        }
      ...
      d->displayText("Loading next movie, please wait...");
      ...
      d->setDesiredRefreshDelayUsec(itsMovieFrameDelay.getVal());
      ...
      d->pushEvent(sformat("Start audio recording file %d", recnb));
      recordaudio = true;
      ...
      while(cache.size())
      {
          ...
          //if requested, play a beep on each frame
          if (beepon)
            a->playBeep();

          if (frame == 0)
            {
              d->displayVideoOverlay(vidframe, frame, SDLdisplay::NO_WAIT);
              d->waitNextRequestedVsync(true, true);
            }
          else
      ...
      d->setDesiredRefreshDelayUsec(itsSDLRefresh.getVal());
      ...
      d->pushEvent(sformat("Stop audio recording: audio%04d.wav", recnb));
      recordaudio = false;
