...
// 
// see /lab/nnoori/works/experimetns/twsr/paradigm.txt for more details about the design of paradigm
// we will show 6 items for memory test either horizontally or vertically and test memory recall for either forward recall or backward recall
...
ModelManager manager("Psycho-Serial-Recall");
...
void showBoxesHorizontally(uint numofboxes  , int wsd , string ds){
  
  int x = (d->getWidth()-numofboxes*wsd)/2 ;
        int y = (d->getHeight())/2 -10;
        Image<PixRGB<byte> > textIm(d->getWidth(),d->getHeight(),ZEROS);
        textIm.clear(PixRGB<byte>(128,128,128));
        for( uint k = 0 ; k < numofboxes ; k++ ){
               // d->displayText(s.substr(k,1),Point2D<int>(x,y+k*10),PixRGB<byte>(0,0,0),PixRGB<byte>(128,128,128),true) ; 
                writeText(textIm, Point2D<int>(x+k*wsd,y),ds.c_str(),PixRGB<byte>(0,0,0),PixRGB<byte>(128,128,128));
        }
        SDL_Surface *surf = d->makeBlittableSurface(textIm , true);
        SDL_Rect offs ; offs.x = 0 ; offs.y = 0 ;
        d->displaySDLSurfacePatch(surf , &offs , NULL , -2 , false ,true ) ;
...
void showBoxesVertically(uint numofboxes , int wsd, string ds ){
  ...
  int x = (d->getWidth()-numofboxes*wsd)/2 ;
        ...
        for( uint k = 0 ; k < numofboxes ; k++ ){
               // d->displayText(s.substr(k,1),Point2D<int>(x,y+k*10),PixRGB<byte>(0,0,0),PixRGB<byte>(128,128,128),true) ; 
                writeText(textIm, Point2D<int>(x,y+k*wsd),ds.c_str(),PixRGB<byte>(0,0,0),PixRGB<byte>(128,128,128));
        }
        SDL_Surface *surf = d->makeBlittableSurface(textIm , true);
        SDL_Rect offs ; offs.x = 0 ; offs.y = 0 ;
        d->displaySDLSurfacePatch(surf , &offs , NULL , -2 , false ,true ) ;
...
void flashDisplayHorizontally(string box, string s , int l , int onsetTime , int wsd , uint p){
        string tmpstr="";
        for (int i = 0 ; i < l ; i++) tmpstr += box;
        tmpstr.replace(p,1,s);
        int x = (d->getWidth()-s.size()*wsd)/2 ;
        int y = (d->getHeight())/2 -10;
        Image<PixRGB<byte> > textIm(d->getWidth(),d->getHeight(),ZEROS);
        textIm.clear(PixRGB<byte>(128,128,128));
        for( uint k = 0 ; k < tmpstr.size() ; k++ ){
                writeText(textIm, Point2D<int>(x+k*wsd,y),tmpstr.substr(k,1).c_str(),PixRGB<byte>(0,0,0),PixRGB<byte>(128,128,128));
        }
        SDL_Surface *surf = d->makeBlittableSurface(textIm , true);
        SDL_Rect offs ; offs.x = 0 ; offs.y = 0 ;
        d->displaySDLSurfacePatch(surf , &offs , NULL , -2 , false ,true ) ;
...
void flashDisplayVertically(string box, string s , int l , int onsetTime , int wsd , uint p){
        string tmpstr="";
        for (int i = 0 ; i < l ; i++) tmpstr += box;
        tmpstr.replace(p,1,s);
        int x = (d->getWidth()-s.size()*wsd)/2 ;
        ...
        textIm.clear(PixRGB<byte>(128,128,128));
        for( uint k = 0 ; k < tmpstr.size() ; k++ ){
                writeText(textIm, Point2D<int>(x,y+k*wsd),tmpstr.substr(k,1).c_str(),PixRGB<byte>(0,0,0),PixRGB<byte>(128,128,128));
        }
        SDL_Surface *surf = d->makeBlittableSurface(textIm , true);
        SDL_Rect offs ; offs.x = 0 ; offs.y = 0 ;
        d->displaySDLSurfacePatch(surf , &offs , NULL , -2 , false ,true ) ;
...
extern "C" int main(const int argc, char** argv)
{
        ...
        argMap["experiment"]="twsr";
        argMap["logfile"]="psycho-twsr.psy" ;
        argMap["string-size"]="6" ;
        argMap["num-of-trials"]="10";
        argMap["subject"]="" ;
        argMap["memo"]="" ;
       	argMap["alphabet"]="bfgkms";
        argMap["mode"]="1" ;
        argMap["sound-dir"]="..";
	    argMap["tone1"]="sine";
	    argMap["tone2"]="square";
        ...
        argMap["box-sign"]=".";
        argMap["stimulus-onset-frames"]="20";
  ...
  // let's display an ISCAN calibration grid:
	    ...
	    int mode = atoi(argMap["mode"].c_str());
 	    ...
 	    int max_tone_wait = atoi(argMap["max-tone-wait"].c_str());
            ...
            int white_space = atoi(argMap["white-space"].c_str());
             int stimulus_onset_frames = atoi(argMap["stimulus-onset-frames"].c_str());
	    vector<int> orVector;
    ...
    int missCounting=0;
	    cr=0;
            while( cr < numOfTests ){
                string inStr = getARandomString(stringSize,"dfjkmr");
                d->showCursor(false);
                if(mode==1) {
                  showBoxesHorizontally(6, white_space,"." )
                }else{
                  showBoxesVertically(6, white_space,"." )
                }
                d->clearScreen() ;
                d->pushEvent("**************************************") ;
                d->pushEvent("initial string :" + inStr);
                for(uint i = 0 ;  i < inStr.size(); i++){
                    d->pushEvent("displaying character "+inStr.substr(i,1));
                    if(mode==1){
                      flashDisplayHorizontally(".", inStr.sustr(i,1) , stringSize , stimulus_onset_frames , white_space , i);
                    }else{
                       flashDisplayVertically(".", inStr.sustr(i,1) , stringSize , stimulus_onset_frames , white_space , i);
                    }
