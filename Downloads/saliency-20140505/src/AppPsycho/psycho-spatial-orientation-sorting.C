/*!@file AppPsycho/psycho-spatial-orientation-sorting.C Psychophysics test to measure the effect of executive tasks on spatial memory */
...
string getDigitSequenceFromSubject(string alphabet="0123456789" , uint maxl = 7 , string message="") {
    ...
    SDL_Rect msgoffs ;
    msgoffs.x = (d->getWidth() - msgp->w) /2 ;
    msgoffs.y = 2*d->getHeight()/9 ;
...
void displayLinear(string s , int onsetTime) {
    int x = (d->getWidth()-s.size()*40)/2 ;
    int y = d->getHeight()/2 - 10;
    for ( uint k = 0 ; k < s.size() ; k++ ) {
        d->displayText(s.substr(k,1),Point2D<int>(x+k*40,y),PixRGB<byte>(0,0,0),PixRGB<byte>(128,128,128),true) ;
        d->waitFrames(onsetTime);
        d->clearScreen() ;
    }
}
...
string get0213x(string is) {
    string sst = ascSort(is);
    string tail= "";
    if (is.size()>4) tail = sst.substr(4,sst.size()-4);
    string rSt= sst.substr(0,1)+sst.substr(2,1)+sst.substr(1,1)+sst.substr(3,1) +tail;
    return rSt ;
}

string get3120x(string is) {
    string sst = ascSort(is);
    string tail= "";
    if (is.size()>4) tail = sst.substr(4,sst.size()-4);
    string rSt= sst.substr(3,1)+sst.substr(1,1)+sst.substr(2,1)+sst.substr(0,1) +tail;
    return rSt ;
}

string get3021x(string is) {
    string sst = ascSort(is);
    string tail= "";
    if (is.size()>4) tail = sst.substr(4,sst.size()-4);
    string rSt= sst.substr(3,1)+sst.substr(0,1)+sst.substr(2,1)+sst.substr(1,1) +tail;
    return rSt ;
}

string get1203x(string is) {
    string sst = ascSort(is);
    string tail= "";
    if (is.size()>4) tail = sst.substr(4,sst.size()-4);
    string rSt= sst.substr(1,1)+sst.substr(2,1)+sst.substr(0,1)+sst.substr(3,1) +tail;
    return rSt ;
}

string get1032x(string is) {
    string sst = ascSort(is);
    string tail= "";
    if (is.size()>4) tail = sst.substr(4,sst.size()-4);
    string rSt= sst.substr(1,1)+sst.substr(0,1)+sst.substr(3,1)+sst.substr(2,1) +tail;
    return rSt ;
}

string get2301x(string is) {
    string sst = ascSort(is);
    string tail= "";
    if (is.size()>4) tail = sst.substr(4,sst.size()-4);
    string rSt= sst.substr(2,1)+sst.substr(3,1)+sst.substr(0,1)+sst.substr(1,1) +tail;
    return rSt ;
}
...
int spatial_memory_retrival(vector<int> dotsVector,int dir ,int ma=30 , int Ma=120 ,int ds0=30 , int ds1=30) {

    int w = d->getWidth();
    int h = d->getHeight();
    int change = rand()%2;
    int neg = dotsVector.at(0);
    int pos = dotsVector.at(1);
	int ds = 0 ;
	if (dir == 0) ds = ds0 ;
	if (dir == 1) ds = ds1 ;
    ...
    switch ( change ) {
    case 0 :
        d->pushEvent("same target");
        break;
    case 1 :
        d->pushEvent("target changed");
        break;
    }
    if (change == 1) {
        int chDir = rand()%2;
...
std::string getUsageComment() {
    ...
    com += "\ndot-shift=[>1](amount of shift in dots position in pixel)[default=32]\n";
	com += "\ndot-shift-horizontal=[>1](amount of shift in dots position along the horizontal direction in pixels)[default=dot-shift]\n";
	com += "\ndot-shift-vertical=[>1](amount of shift in dots position along the vertical direction in pixel)[default=dot-shift]\n";
...
extern "C" int main(const int argc, char** argv)
{
    ...
    argMap["dot-shift"] = "32";
	argMap["dot-shift-horizontal"] = "0";
	argMap["dot-shift-vertical"] = "0";
    argMap["sorting-delay"]="300";
	...
	int dot_max_change_h = atoi(argMap["dot-shift-horizontal"].c_str());
	int dot_max_change_v = atoi(argMap["dot-shift-vertical"].c_str());
	if (dot_max_change_h == 0 ) dot_max_change_h = dot_max_change ;
	if (dot_max_change_v == 0 ) dot_max_change_v = dot_max_change ;
    vector<int> orientation_task_Vector;
    for ( int i = 0 ; i< numOfTests ; i++ ) {
        orientation_task_Vector.push_back(0);
        orientation_task_Vector.push_back(1);
        //orientation_task_Vector.push_back(2);
        //orientation_task_Vector.push_back(3);
    }
    ...
    map<int,int> correctSPMMap;
    correctSPMMap[0]=0;
    correctSPMMap[1]=0;
    correctSPMMap[2]=0;
    correctSPMMap[3]=0;
    map<int,int> incorrectSPMMap;
    incorrectSPMMap[0]=0;
    incorrectSPMMap[1]=0;
    incorrectSPMMap[2]=0;
    incorrectSPMMap[3]=0;
    ...
    while ( cr < 2*numOfTests ) {
        string inStr = getARandomString(stringSize,argMap["alphabet"]);

        string testString="" ;
        int flp = rand()%6;
        switch ( flp ) {
        case 0 :
            testString = get0213x(ascSort(inStr));
            break;
        case 1 :
            testString = get3120x(ascSort(inStr));
            break;
        case 2 :
            testString = get3021x(ascSort(inStr));
            break;
        case 3 :
            testString = get1203x(ascSort(inStr));
            break;
        case 4 :
            testString = get1032x(ascSort(inStr));
            break;
        case 5 :
            testString = get2301x(ascSort(inStr));
            break;
        }
        d->showCursor(false);
        d->clearScreen() ;
        ...
        d->pushEvent("**************************************") ;
        d->pushEvent("initial string :" + testString);
        for ( int i = 0 ;  i < stringSize ; i++ ) {
            playTone(audio_map[charmap[testString.substr(i,1)]]);
            d->waitFrames(5);
        }
        //displayRandom(testString,15);
        //displayWholeNumber(testString,60,20);
        showMask(4,"abcdefghijklmnopqrstuvwxyz");
        d->waitFrames(30);
        d->displayFixation();
        d->waitFrames(10);
        d->clearScreen() ;
        ...
        /*switch(orientation_task_Vector.at(cr)){
            case 0 : currentTask=0 ;break;
            case 1 : currentTask=1 ;break;
            case 2 : currentTask=0 ;break;
            case 3 : currentTask=1 ;break;
        }     */
        if (currentTask==0) {
            d->pushEvent("task : memorization");
            playTone(tone1);
        }
        if (currentTask==1) {
            d->pushEvent("task : sorting");
            playTone(tone1);
        }
         if (currentTask==1) {
            d->pushEvent("task : reversing");
            playTone(tone1);
        }
        ...
        int spMemAns = spatial_memory_retrival(dotsVector,(int)(orientation_task_Vector.at(cr)%2), dots_min_radius, dots_max_radius, dot_max_change_h, dot_max_change_v);
        switch ( spMemAns ) {
        case 0:
            incorrectSPMMap[orientation_task_Vector.at(cr)]= incorrectSPMMap[orientation_task_Vector.at(cr)]+1;
            d->pushEvent("incorrect spatial memory answer");
            break;
        case 1:
            correctSPMMap[orientation_task_Vector.at(cr)]= correctSPMMap[orientation_task_Vector.at(cr)]+1;
            d->pushEvent("correct spatial memory answer");
            break;
        }

        string ans1 = getDigitSequenceFromSubject(argMap["alphabet"],stringSize,"");
        d->pushEvent("string answer: "+ans1);
        if (mode==0) {
            if (ans1.compare(testString)==0) {
                d->pushEvent("correct string recall");
                d->displayText(":)");
                d->waitFrames(15);
            } else {
                d->pushEvent("incorrect string recall");
                d->displayText(":(");
                d->waitFrames(15);
            }
        }
        ...
        if (mode==1) {
            if (ans1.compare(ascSort(testString))==0) {
                d->pushEvent("correct string sorting");
                d->displayText(":)");
                d->waitFrames(15);
            } else {
                d->pushEvent("incorrect string sorting");
                d->displayText(":(");
                d->waitFrames(15);
            }
        }
		 if (mode==2) {
            if (ans1.compare(reverseStr(testString))==0) {
                d->pushEvent("correct string reversing");
                d->displayText(":)");
                d->waitFrames(15);
            } else {
                d->pushEvent("incorrect string reversing");
                d->displayText(":(");
                d->waitFrames(15);
            }
        }
