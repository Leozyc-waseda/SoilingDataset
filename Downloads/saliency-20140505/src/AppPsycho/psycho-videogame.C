...
// $Id: psycho-video.C 13539 2010-06-09 22:43:26Z ilab19 $
//
...
int submain(const int argc, char** argv)
{
  ...
  // A new UDP sync @ iWin3 machine <<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<
  // This is for the hand tracker machine, the raw ip is below
  //PsychoUDP * pUDP = new PsychoUDP("iWin3", "45454");
  PsychoUDP * pUDP = new PsychoUDP("192.168.0.224", "45454");
  //PsychoUDP * pUDP = new PsychoUDP("192.168.0.128", "45454");
  ...
  snprintf(msg, 300, "== START TIME: %d:%02d:%02d.%03" ZU ".%03" ZU " ==",
  ...
  snprintf(msg, 300, "== START RECORDING: %d:%02d:%02d.%03" ZU ".%03" ZU " ==",
           tm->tm_hour, tm->tm_min, tm->tm_sec, tv.tv_usec / 1000,
           tv.tv_usec % 1000);
  d->pushEvent(msg);

  // Send data to hand tracker that we start recording
  pUDP->sendData("record");
  ...
  while (framenum < maxframe)
    {
      ...
      // grab a raw buffer:
      GenericFrame rawframe=gb->readFrame();
      const VideoFrame frame = rawframe.asVideo();
      if(framenum%2 == 0)
        ds->writeFrame(rawframe, "frame");

      // display the frame as an overlay
      d->displayVideoOverlay(frame, framenum, SDLdisplay::NO_WAIT);

      // for hand synchronization
      if ((framenum % 100) == 99) {
        snprintf(msg, 300, "f%06d", framenum);
        pUDP->sendData(msg);
      }
  ...
  // sync stop signal to handtracker
  pUDP->sendData("done");
