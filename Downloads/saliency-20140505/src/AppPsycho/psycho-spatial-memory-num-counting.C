/*!@file AppPsycho/psycho-spatial-memory-num-counting.C Psychophysics test to measure the effect of executive tasks on spatial memory */
...
// $HeadURL: svn://isvn.usc.edu/software/invt/trunk/saliency/src/AppPsycho/psycho-spatial-memory-num-counting.C $
// $Id: psycho-spatial-memory-num-counting.C 13059 2010-03-26 08:14:32Z itti $
//
...
extern "C" int main(const int argc, char** argv)
{
	    ...
	    argMap["min-tone-wait-1"]="40";
	    argMap["max-tone-wait-1"]="60";
	    argMap["min-tone-wait-2"]="90";
	    argMap["max-tone-wait-2"]="110";
  ...
  // let's display an ISCAN calibration grid:
	    ...
	    int mode = atoi(argMap["mode"].c_str());
 	    ...
 	    int min_tone_wait_1 = atoi(argMap["min-tone-wait-1"].c_str());
 	    int max_tone_wait_1 = atoi(argMap["max-tone-wait-1"].c_str());
	    int min_tone_wait_2 = atoi(argMap["min-tone-wait-2"].c_str());
	    int max_tone_wait_2 = atoi(argMap["max-tone-wait-2"].c_str());
	...
	int correctSpatialMemory_1=0;
	int incorrectSpatialMemory_1=0;
	int correctSpatialMemory_2=0;
	int incorrectSpatialMemory_2=0;
	int correctCounting=0;
	int incorrectCounting=0;
    int missCounting=0;
    	vector<int> tempoVector;
	for( int i = 0 ; i < numOfTests /2 ;i++ ){
		tempoVector.push_back(0);
	}
	for( int i = numOfTests /2 ; i < numOfTests ;i++ ){
		tempoVector.push_back(1);
	}
	scramble(tempoVector);
	    cr=0;
            while( cr < numOfTests ){
                string inStr = getARandomString(stringSize,"234567");
		d->showCursor(false);
		d->clearScreen() ;
		d->pushEvent("**************************************") ;
		if(tempoVector.at(cr)==0) d->pushEvent("fast tempo");
		if(tempoVector.at(cr)==1) d->pushEvent("slow tempo");
		...
		d->waitFrames(10);
		d->clearScreen() ;
                vector<int> angles = spatial_memory_task(dot_onset,dots_ISI,dots_radius,dots_min_angle,dots_max_angle);
		d->pushEvent("dots showed at these angles :"+ stringify(angles.at(0))+" and " + stringify(angles.at(1)));
		d->waitFrames(10);
		d->displayFixation();
		vector<int> tonesVector;
		if(tempoVector.at(cr)==0)
		 tonesVector = executive_memory_task( tone1 , tone2 , inStr, counter_length, min_tone_wait_1, max_tone_wait_1 );
		if(tempoVector.at(cr)==1)
		 tonesVector = executive_memory_task( tone1 , tone2 , inStr, counter_length, min_tone_wait_2, max_tone_wait_2 );
		string  newStr ;
		if (stringSize == 2)
		...
		if(mode==1 || mode==3){
			int spMemAns = spatial_memory_retrival(angles,dots_radius);
			if(tempoVector.at(cr)==0){
				switch( spMemAns ){
					case 0 : incorrectSpatialMemory_1++;d->pushEvent("incorrect spatial memory recall");break;
					case 1 : correctSpatialMemory_1++;d->pushEvent("correct spatial memory recall");break;
				}
			}
			
			if(tempoVector.at(cr)==1){
				switch( spMemAns ){
					case 0 : incorrectSpatialMemory_2++;d->pushEvent("incorrect spatial memory recall");break;
					case 1 : correctSpatialMemory_2++;d->pushEvent("correct spatial memory recall");break;
				}
			}
		}
	    ...
	    d->pushEvent("correct spatial memory (fast tempo): " +stringify(correctSpatialMemory_1));
	    d->pushEvent("incorrect spatial memory (fast tempo): " +stringify(incorrectSpatialMemory_1));
	    d->pushEvent("correct spatial memory (slow tempo): " +stringify(correctSpatialMemory_2));
	    d->pushEvent("incorrect spatial memory (slow tempo): " +stringify(incorrectSpatialMemory_2));
