/*!@file AppPsycho/psycho-spatial-memory.C Psychophysics test to measure the effect of executive tasks on spatial memory */
...
// $Id: psycho-spatial-memory.C 14784 2011-05-27 10:59:02Z nnoori $
//
...
//////////////////////////////////////////////////////
string digitMemorizationTask(nub::soft_ref<PsychoDisplay> d, uint l, string alphabet="0123456789" , int displayFrame = 10  ) {
    d->clearScreen() ;
    vector<int> pickedones = vector<int>() ;
    string test = string("") ;
    string tp = string("") ;
    for (uint i = 0 ; i < l ; i++) {
        ...
        do {
            nd= rand()% alphabet.size() ;
        }while (itIsInThere(nd,pickedones) && pickedones.size() <= alphabet.size()) ;
        pickedones.push_back(nd);
        tp = alphabet.substr(nd,1) ;
        test += tp ;
    }
...
SDL_Surface* getKeyPad(nub::soft_ref<PsychoDisplay> d, string alphabet,map<string , SDL_Rect>& buttmap) {
    ...
    for ( int i = 0 ; i < numofrows*3 ; i++) {
        ...
        if ((uint)i < alphabet.size()) {
            but = getButtonImage(d,alphabet.substr(i,1),PixRGB<byte>(0,0,0),PixRGB<byte>(255,255,255),Point2D<int>(pad->w / numofcolumns , pad->h / numofrows),PixRGB<byte>(255, 98 , 25),3);
        } else {
        ...
        SDL_Rect cl ;
        cl.x = ((i)%numofcolumns)*(pad->w)/numofcolumns ;
        cl.y= ((i)/numofcolumns)*((pad->h)/numofrows) ;
        cl.w = clip.w ;
        cl.h = clip.h ;
    ...
    SDL_Rect cl1 ;
    cl1.x = 0 ;
    cl1.y= (numofrows-1)*((pad->h)/numofrows) ;
    cl1.w = clip.w ;
    cl1.h = clip.h ;
    buttmap["!"] = cl1 ;
    ...
    dumpSurface(but);
    SDL_Rect cl2 ;
    cl2.x = (pad->w)/numofcolumns ;
    cl2.y= (numofrows-1)*((pad->h)/numofrows) ;
    cl2.w = clip.w ;
    cl2.h = clip.h ;
    buttmap[" "] = cl2 ;
    ...
    dumpSurface(but);
    SDL_Rect cl3 ;
    cl3.x = 2*(pad->w)/numofcolumns ;
    cl3.y= (numofrows-1)*((pad->h)/numofrows) ;
    cl3.w = clip.w ;
    cl3.h = clip.h ;
    buttmap["*"] = cl3 ;
...
////////////////////////////////////////////////////
string getDigitSequenceFromSubject(nub::soft_ref<PsychoDisplay> d,string alphabet="0123456789" , uint maxl = 7 ) {
    d->showCursor(true) ;
    ...
    SDL_Surface * keypad = getKeyPad(d,alphabet,*buttmap);
    ...
    while ( tp.compare("*")!=0 ) {
        ...
        SDL_Surface* dp = getButtonImage(d,p ,PixRGB<byte>(195,60,12) ,PixRGB<byte>(255,255,255) ,Point2D<int>(d->getWidth()/6,d->getHeight() /15) ,PixRGB<byte>(0,25,180) , 4) ;
        SDL_Rect offs ;
        offs.x = (d->getWidth() - dp->w) /2 ;
        offs.y = d->getHeight()/6 ;
...
void displayWholeNumber(nub::soft_ref<PsychoDisplay> d,string s , int onsetTime , int wsd) {
...
void displayWholeNumberVertically(nub::soft_ref<PsychoDisplay> d,string s , int onsetTime , int wsd) {
    int x = (d->getWidth())/2 ;
...
void displayRandom(nub::soft_ref<PsychoDisplay> d,string s , int onsetTime) {
    for ( uint k = 0 ; k < s.size() ; k++ ) {
...
void displayLinear(nub::soft_ref<PsychoDisplay> d,string s , int onsetTime) {
...
void displayLinearReverse(nub::soft_ref<PsychoDisplay> d,string s , int onsetTime) {
    int x = (d->getWidth()-s.size()*10)/2 ;
    int y = d->getHeight()/2 - 10;
    for ( uint k = 0 ; k < s.size() ; k++ ) {
...
void displayLinearRandom(nub::soft_ref<PsychoDisplay> d,string s , int onsetTime) {
    int x = (d->getWidth()-s.size()*10)/2 ;
    int y = d->getHeight()/2 - 10;
    for ( uint k = 0 ; k < s.size() ; k++ ) {
...
void displayLinearRandomVertically(nub::soft_ref<PsychoDisplay> d,string s , int onsetTime) {
    int x = (d->getWidth())/2 ;
...
void displayLinearRandomNoRepeating(nub::soft_ref<PsychoDisplay> d,string s , int onsetTime) {
    int x = (d->getWidth()-s.size()*10)/2 ;
    int y = d->getHeight()/2 - 10;
    for ( uint k = 0 ; k < s.size() ; k++ ) {
        d->displayText(s.substr(k,1),Point2D<int>(x+ (random()%s.size())*10,y),PixRGB<byte>(0,0,0),PixRGB<byte>(128,128,128),true) ;
...
void showMask(nub::soft_ref<PsychoDisplay> d,int frames, string alphabet="0123456789") {
    Image<PixRGB<byte> > textIm(d->getWidth(),d->getHeight(),ZEROS);
...
vector<int> spatial_memory_task(nub::soft_ref<PsychoDisplay> d,int onsetTime, int isd ,int rad=100 ,  int ma=45 ,int Ma=90) {

    int w = d->getWidth();
    int h = d->getHeight();
    int a1 = rand() % 360 ;
    int direction = (int)(((rand()%2)-0.5)/0.5);
    int a2 =a1 +direction*((rand() % (Ma-ma))+ma);
    //d->displayFixation();
    d->displayRedDotFixation(w/2+rad*cos(a1*3.1415/180),h/2 + rad*sin(a1*3.1415/180));
    d->waitFrames(onsetTime);
    d->clearScreen();
    //d->displayFixation();
    d->waitFrames(isd);
    d->displayRedDotFixation(w/2+rad*cos(a2*3.1415/180),h/2 + rad*sin(a2*3.1415/180));
    d->waitFrames(onsetTime);
    d->clearScreen();
    vector<int> angles;
    angles.push_back(a1);
...
int spatial_memory_retrival(nub::soft_ref<PsychoDisplay> d,vector<int> angles ,int rad = 100 ) {
    int res=0 ;
    int flip = rand()%2;
    int w = d->getWidth();
    int h = d->getHeight();
    if (flip==0) {
        d->displayRedDotFixation(w/2+rad*cos(angles.at(0)*3.1415/180),h/2 + rad*sin(angles.at(0)*3.1415/180));
        d->displayRedDotFixation(w/2+rad*cos(angles.at(1)*3.1415/180),h/2 + rad*sin(angles.at(1)*3.1415/180));
    } else {
        int a1=angles.at(0) % 360;
        int a2=angles.at(1) % 360;
        if (a2<a1) {
            int tmp = a2 ;
            a2=a1;
            a1=tmp;
    ...
    d->waitFrames(20);
    d->clearScreen();
    string ans = getDigitSequenceFromSubject(d,"yn");
    if (flip==0) {
        res = 0 ;
        if (ans.compare("y")==0) res=1;
    } else {
        res = 0 ;
        if (ans.compare("n")==0) res=1;
    }
    return res ;
}

vector<int> executive_memory_task(nub::soft_ref<PsychoDisplay> d,Mix_Music* tone1 , Mix_Music* tone2 ,string initial_string,int counter_length, int min_tone_wait, int max_tone_wait ) {
    int t1c = 0 ;
    int t2c = 0 ;
    vector<int> wFrames;
...
extern "C" int main(const int argc, char** argv)
{
    ...
    argMap["alphabet"]="abcdefghijklmnopqrstuvwxyz";
    argMap["mode"]="1" ;
    ...
    argMap["min-tone-wait"]="60";
    argMap["max-tone-wait"]="90";
    argMap["dots_radius"]="100";
    argMap["dots_min_angle"]="45";
    argMap["dots_max_angle"]="90";
    ...
    int dots_radius = atoi(argMap["dots_radius"].c_str());
    int dots_min_angle = atoi(argMap["dots_min_angle"].c_str());
    int dots_max_angle = atoi(argMap["dots_max_angle"].c_str());
    int mode = atoi(argMap["mode"].c_str());
    ...
    int correctSpatialMemory=0;
    int incorrectSpatialMemory=0;
    int correctCounting=0;
    int incorrectCounting=0;
    int missCounting=0;
    cr=0;
    while ( cr < numOfTests ) {
        string inStr = getARandomString(stringSize,"cdfgh");
        d->showCursor(false);
        d->clearScreen() ;
        d->pushEvent("**************************************") ;
        ...
        for (uint i = 0 ;  i < inStr.size(); i++) {
            ...
            if (Mix_PlayMusic(audio_map[charmap[inStr.substr(i,1)]],0)==-1) {
                return 1;
            }
            while (Mix_PlayingMusic()==1) {} ;
        ...
        d->waitFrames(10);
        d->clearScreen() ;
        vector<int> angles = spatial_memory_task(d,dot_onset,dots_ISI,dots_radius,dots_min_angle,dots_max_angle);
        d->pushEvent("dots showed at these angles :"+ stringify(angles.at(0))+" and " + stringify(angles.at(1)));
        d->waitFrames(10);
        d->displayFixation();
        vector<int> tonesVector = executive_memory_task(d, tone1 , tone2 , inStr, counter_length, min_tone_wait, max_tone_wait );
        string  newStr ;
        if (stringSize == 2)
        ...
        if (mode==1 || mode==3) {
            int spMemAns = spatial_memory_retrival(d,angles,dots_radius);
            switch ( spMemAns ) {
            case 0 :
                incorrectSpatialMemory++;
                break;
            case 1 :
                correctSpatialMemory++;
                break;
            }
        }

        string ans = getDigitSequenceFromSubject(d,masterString);
        d->pushEvent("subject keyed:" + ans);
        if (mode ==1) {
            if (ans.compare(inStr)==0) {
                correctCounting++;
            } else {
                incorrectCounting++ ;
            }
        ...
        if (mode ==2 || mode==3) {
            d->pushEvent("new string is "+ newStr);
            if (ans.compare(newStr)==0) {
                correctCounting++;
            } else {
                incorrectCounting++ ;
                int n1 = charmap[newStr.substr(0,1)];
                int n2 = charmap[newStr.substr(newStr.size()-1,1)];
                missCounting += abs(n1-tonesVector.at(0)) + abs(n2-tonesVector.at(1)) ;
            }

        }
        cr++;
    }
    d->pushEvent("correct spatial memory: " +stringify(correctSpatialMemory));
    d->pushEvent("incorrect spatial memory: " +stringify(incorrectSpatialMemory));
    d->pushEvent("correct counting: " +stringify(correctCounting));
    d->pushEvent("incorrect counting: " +stringify(incorrectCounting));
    d->pushEvent("misscounting: "+stringify(missCounting));
