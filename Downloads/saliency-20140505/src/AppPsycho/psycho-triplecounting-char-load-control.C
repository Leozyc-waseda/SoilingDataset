/*!@file AppPsycho/psycho-triplecounting-char-load-control.C  stimulus presentation program for parity test mental task*/ 
...
// $HeadURL: svn://isvn.usc.edu/software/invt/trunk/saliency/src/Psycho/psycho-triplecounting-char-load-control.C$
...
void performSpatialCueingBlock(nub::soft_ref<PsychoDisplay> d, int numOfTrials ,int numOfTestItems ){
  ...
  d-> displayText(" BOX counter retaining,  "+stringify<int>(numOfTrials)+" trials in a row!");
  d-> waitForKey();
  int cr = 0 ; 
  while(cr < numOfTrials){
    ...
    for(int i = 0 ; i < numOfTestItems ; i++ )initialCounters.push_back(rand()%10);
...
void performCharcterCueingBlock(nub::soft_ref<PsychoDisplay> d, int numOfTrials , int numOfTestItems){
   d-> displayText("Character counters retaining : "+stringify<int>(numOfTrials)+" trials in a row!");
   d->waitForKey();
  int cr = 0 ; 
  while(cr < numOfTrials){
