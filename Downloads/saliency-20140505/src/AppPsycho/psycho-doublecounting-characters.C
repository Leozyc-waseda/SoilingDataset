/*!@file AppPsycho/psycho-doublecounting-characters.C  stimulus presentation program for parity test mental task*/ 
...
// $HeadURL: svn://isvn.usc.edu/software/invt/trunk/saliency/src/Psycho/psycho-doublecounting-characters.C$
...
void performSpatialCueingBlock(nub::soft_ref<PsychoDisplay> d, int numOfTrials ,int numOfTestItems ){
  ...
  while(cr < numOfTrials){
    ...
    vector<int> myBoxes =  getRandomNonRepeatingNumbers(numOfTestItems,fourCenters.size());
    vector<int> boxCounter ;
    vector< Point2D<int> > boxCenters; 
    vector<string> chars;
    globalTrialObjectOrder.push_back(myBoxes);
    vector<int> initialCounters ;
    for(int i = 0 ; i < numOfTestItems ; i++ )initialCounters.push_back(rand()%3);
    ...
    for(int i = 0 ; i < numOfIncrements ; ){
      int boxIndex = rand()%numOfTestItems ;
      int dice = rand()%(3*numOfTestItems);
      if (dummyChar.size() > 0)  dice = rand()%(numOfTestItems*3 +1);
      int charIndex = dice / 3 ;
    ...
    do{
      ...
      for (int j = 1 ; j <= repeatBreakPoint ; j++){
          if (boxIndexHistory.at(boxIndexHistory.size()-j)!= boxIndex) repeatFlag = 0 ; 
      }
    }while(repeatFlag == 1);
    
    repeatFlag = 1 ;
    do{
      dice = rand()%(3*numOfTestItems);
      if (dummyChar.size() > 0)  dice = rand()%(numOfTestItems*3 +1);
      charIndex = dice / 3 ;
      for (int j = 1 ; j <= repeatBreakPoint ; j++){
          if (charIndexHistory.at(charIndexHistory.size()-j)!= charIndex) repeatFlag = 0 ; 
      }
    }while(repeatFlag == 1);
      }
...
void performCharcterCueingBlock(nub::soft_ref<PsychoDisplay> d, int numOfTrials , int numOfTestItems){
...
//     fourCenters.push_back(Point2D<int>(d->getWidth()/2 + boxDistance , d->getHeight()/2 - boxDistance));
  ...
  fourCenters.push_back(Point2D<int>(d->getWidth()/2 + boxDistance , d->getHeight()/2 ));
    ...
    for(int i = 0 ; i < numOfBlocks ; i++){
      if(mode == 0){
    performSpatialCueingBlock(d,numOfTrials,numOfCountingItems);
    performCharcterCueingBlock(d,numOfTrials,numOfCountingItems);
      }else{
    performCharcterCueingBlock(d,numOfTrials,numOfCountingItems);
    performSpatialCueingBlock(d,numOfTrials,numOfCountingItems);
      }
