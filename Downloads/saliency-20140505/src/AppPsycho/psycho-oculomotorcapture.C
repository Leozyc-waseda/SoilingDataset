...
static int submain(const int argc, char** argv)
{
  ...
  d->waitForKey(true);
	...
	const PixRGB<byte> white = PixRGB<byte>(255,255,255); // distractor color (red)
	...
	// main loop
	double delay;
	int j, iscontrol, ang, thisTarget, thisDistractor, thisSOA;
	...
	for (int i=0; i < ntrial; i++) {
		...
		// track eye
		d->displayFilledCircle(x, y, 3, white, true);
		usleep(900000);
		et->track(true);
		usleep(100000);

		// display all possible target locations
  	filledCircleRGBA(surf, x, y, 3, white.red(), white.green(), white.blue(), 0XFF);
		for (j=0; j<6; j++){
		...
		// show direction
		ang = 45 - 60 * thisTarget - 180; 
		SDL_Surface *tmpsurf = rotozoomSurface(arrowsurf, ang, 1, 0.5);
		rr.x = d->getWidth()/2-tmpsurf->w/2; rr.y = d->getHeight()/2-tmpsurf->h/2;
		SDL_BlitSurface(tmpsurf, NULL, surf, &rr); 
