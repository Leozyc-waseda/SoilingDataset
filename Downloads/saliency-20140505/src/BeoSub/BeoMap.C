/*!@file BeoSub/BeoMap.C Class for stitching images */
...
// $HeadURL: svn://isvn.usc.edu/software/invt/trunk/saliency/src/BeoSub/BeoMap.C $
// $Id: BeoMap.C 14376 2011-01-11 02:44:34Z pez $
//

#include "BeoSub/BeoMap.H"

// ######################################################################
BeoMap::BeoMap(float cut_thresh, int size_thresh, bool toCut)
{
...
// ######################################################################
void BeoMap::makePanorama(const char* nam1, const char* nam2)
{
  MYLOGVERB = LOG_INFO;
  ...
  LINFO("keypoint extractions completed for input image (%d keypoints)",
        vo1->numKeypoints());
  ...
  for (int i = 1; i < 2; i ++)
  {
      ...
      if(!cut){
         ...
         votemp( new VisualObject(nam2, "", im2));
        vo2 = votemp;
          }
      else{
        rutz::shared_ptr<VisualObject>
        votemp( new VisualObject(nam2, "", im3));
        vo2 = votemp;
          }
      */
      ...
      LINFO("keypoint extractions completed for map (%d keypoints)",
            vo2->numKeypoints());
      ...
      //LINFO("modMinX %d, modMaxX %d, modMinY %d, modMaxY %d",
      //modMinX, modMaxX, modMinY, modMaxY);
  ...
  int minStitchedX=0, maxStitchedX=0,
    minStitchedY=0, maxStitchedY=0,
    counterStitching = 0;
  int minStitchedX2=0, maxStitchedX2=0,
    minStitchedY2=0, maxStitchedY2=0,
    counterStitching2 = 0;
  ...
  //  }
    ...
    // w = maxStitchedX2+1;
    // h = maxStitchedY2+1;
  if(  (pano2.getHeight()*pano2.getWidth()/*w*h*/) / (float)((maxStitchedX - minStitchedX+1)*(maxStitchedY - minStitchedY+1)) >= CUT_THRESHOLD )
    {
      ...
      //LINFO("===================================================\n");
        ...
        if( (pano2.getWidth()/*w*/ - medge.rightedge) < 20){
        ...
        if( (pano2.getHeight()/*h*/ - medge.loweredge) < 20){
        ...
        //        LINFO("===================================================\n");
                ...
                int widthtmp(pano2.getWidth()/*w*/); int hswitch(0);
