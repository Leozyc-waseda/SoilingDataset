...
frame* get_frame(void)
{
  ...
  switch (io) {
    case IO_METHOD_READ:
      ...
      return process_image (buffers[0].start, buffers[0].length);
    ...
    case IO_METHOD_MMAP:
      ...
      return process_image (buffers[buf.index].start,buffers[buf.index].length );
    ...
    case IO_METHOD_USERPTR:
      ...
      return process_image ((void *) buf.m.userptr, 0);
...
void init_device(int pix_fmt)
{
  ...
  for (i = 0; ; i ++)
  {
    ...
    unsigned int f;
    for (f = 0; f < (unsigned int)SALGLASSES_VIDFMT_AUTO; f ++)
    ...
    printf("Video Format: Use '%i' for '%s (Fourcc: %c%c%c%c)'%s%s\n",
        f == SALGLASSES_VIDFMT_AUTO ?  -1 : f,
        fdesc.description,
