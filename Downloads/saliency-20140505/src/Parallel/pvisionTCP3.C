/*!@file Parallel/pvisionTCP3.C A parallel vision slave to use w/ pvisionTCP3master */
...
// $HeadURL: svn://isvn.usc.edu/software/invt/trunk/saliency/src/Parallel/pvisionTCP3.C $
// $Id: pvisionTCP3.C 13086 2010-03-30 03:24:43Z beobot $
//

/*! See the pvisionTCP3go script in bin/ for how to launch the slaves, and
  see pvisionTCP3-master.C for the master program. */
  ...
  while(goforever)
    {
      ...
      if (beo->receive(rnode, rmsg, rframe, raction, 3))  // wait up to 3ms
        {
          //LINFO("fFrame %d, action %d from node %d", rframe, raction, rnode);
          tim.reset();
          ...
          switch(raction)
            ...
            case BEO_ORI0:       // ##############################
              ...
              break;
            case BEO_ORI45:      // ##############################
              computeCMAP(rmsg, Oriented5, 45.0, OWEIGHT, 5, beo);
              break;
            case BEO_ORI90:      // ##############################
              computeCMAP(rmsg, Oriented5, 90.0, OWEIGHT, 6, beo);
              break;
            case BEO_ORI135:     // ##############################
