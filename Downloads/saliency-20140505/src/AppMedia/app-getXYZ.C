...
// $HeadURL: svn://isvn.usc.edu/software/invt/trunk/saliency/src/AppMedia/app-getXYZ.C $
// $Id: app-getXYZ.C 10794 2009-02-08 06:21:09Z itti $
//
...
int main(int argc, char** argv)
{
  ...
  Image<PixXYZ<float> > convert =
    static_cast<Image<PixXYZ<float> > >(finput);

  Image<float> foutput;
  foutput.resize(convert.getWidth(),convert.getHeight());

  string affix[3] = {"X","Y","Z"};
  string dot      = ".";
  string type     = "png";
  ...
  for(int i = 0; i < 3; i++)
  {
    ...
    Image<PixXYZ<float> >::iterator iconvert = convert.beginw();
