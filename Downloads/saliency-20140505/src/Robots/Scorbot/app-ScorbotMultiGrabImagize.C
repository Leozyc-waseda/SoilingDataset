...
// ######################################################################
// grabber devices
std::vector< nub::ref<FrameIstream> > grabbers;
...
public:
  ...
  virtual void run() 
  {
    nub::ref<FrameIstream> grabber = grabbers[cameraID];
...
void getInt(const char *msg, int& val)
{
  ...
  char input[1000]; while(fgets(input, 1000, stdin) == NULL) LERROR("Invalid input, try again");
