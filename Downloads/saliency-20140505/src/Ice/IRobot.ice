/*!@file iRobot.ice ice definition file for the irobot */
...
module Robots {
  ...
  enum IRobotModes {SafeMode, FullMode, SpotMode, CoverMode, CoverAndDockMode};
  ...
  interface IRobot {
    ...
    ImageIceMod::ImageIce getImageSensor(short i, bool useColor); 

    //! Get image sersor dims
    ImageIceMod::DimsIce getImageSensorDims(short i);

    //! get sensor values
    float getSensorValue(short i);

    //! Get gps sensor
    bool getSensors(out float xPos, out float yPos, out float ori);

    //! Get change in state 
    bool getDistanceAngle(out float dist, out float ang);
