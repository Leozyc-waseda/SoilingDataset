%clear all;
%function [randUnit] = SimRun(tests)
function [Lx,Ly,Lz,Rx,Ry,Rz] = SimRun(tests)
%function SimRun(tests)
...
for depth = [0:1:75],
    %depth
    R1=[ScreenSize(1)/2-LEDsize(1)/2,ScreenSize(2)/2+LEDsize(2)/2,depth];
    
    [aels{depth+1},aers{depth+1}] = depthIndependentModelSimulation_ret(Ec,d,R1,LEDsize(1),LEDsize(2),ErrorRate,ErrorRate,tests);
end
...
fprintf('Left Eye X: min=%f dist=%d deg=%d\n',MinL(1,3), MinL(1,1), MinL(1,2));
fprintf('Left Eye Y: min=%f dist=%d deg=%d\n',MinL(2,3), MinL(2,1), MinL(2,2));
fprintf('Left Eye Z: min=%f dist=%d deg=%d\n',MinL(3,3), MinL(3,1), MinL(3,2));

fprintf('Right Eye X: min=%f dist=%d deg=%d\n',MinR(1,3), MinR(1,1), MinR(1,2));
fprintf('Right Eye Y: min=%f dist=%d deg=%d\n',MinR(2,3), MinR(2,1), MinR(2,2));
fprintf('Right Eye Z: min=%f dist=%d deg=%d\n',MinR(3,3), MinR(3,1), MinR(3,2));
...
zz = -z;
surf(x,y,zz)
axis([0 75 1 179 -maxval/10000 -minval -maxval/10000 -minval])
hold on;
