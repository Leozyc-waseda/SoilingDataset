function stats = combined_stats_compute_sum_stats(data, frame, feature, class_in, conf)
...
for i=1:size(data,1)
    stats.sum(frame(i,1),stats.new_feature(i,1))         = 0;
    stats.n(frame(i,1),stats.new_feature(i,1))           = 0;
    stats.mean(frame(i,1),stats.new_feature(i,1))        = 0;
    stats.std(frame(i,1),stats.new_feature(i,1))         = 0;
    stats.stderr(frame(i,1),stats.new_feature(i,1))      = 0;
    stats.upper(frame(i,1),stats.new_feature(i,1))       = 0;
    stats.lower(frame(i,1),stats.new_feature(i,1))       = 0;
    stats.bonfcorrect(frame(i,1),stats.new_feature(i,1)) = 0;
    stats.bonferror(frame(i,1),stats.new_feature(i,1))   = 0;
end

for i=1:size(data,1)
     stats.sum(frame(i,1),stats.new_feature(i,1)) = data(i,1)   + stats.sum(frame(i,1),stats.new_feature(i,1));
     stats.n(frame(i,1),stats.new_feature(i,1))   = 1           + stats.n(frame(i,1),stats.new_feature(i,1));
end

stats.mean        = stats.sum .* (1./stats.n);

for i=1:size(data,1)
    stats.std(frame(i,1),stats.new_feature(i,1)) = stats.std(frame(i,1),stats.new_feature(i,1)) + ... 
        (stats.mean(frame(i,1),stats.new_feature(i,1)) - data(i,1))^2;
end
