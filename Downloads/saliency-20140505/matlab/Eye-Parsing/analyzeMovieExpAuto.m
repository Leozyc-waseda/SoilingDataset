function Cdata = analyzeMovieExpAuto(ISCAN, PSYCH, scrw, scrh)
% function CALIBdata = analyzeMovieExpAuto(ISCAN, PSYCH)
...
if(exist(backupFile(1))) %if the first backup file exists
...
while (ii <= ntrials)
  ...
  if ismember(ii,calibtrials)
    ...
    % use backup if it exists - and we did not terminate on this one:
 
    if(exist(backupFile(ii))) % if file exists use it
      ...
      [x1 y1 x2 y2] = parseCalibTraceAuto(cleanTrace(data));
    ...
    if (needcal)
      ...
      try
	if (~exist('calib') | fishycal)
	  fishycal = 0;
	  fprintf(1,'*** Full calibration (n = %i) ***\n',size(ey,2) - 1);  
	  calib = findFullCalibAuto(sc, ey); %fresh new calibration
	else 
	...
	if isempty(in) || lower(in(1)) ~= 'y' % do not force
	  ...
	  in = input(['Skip this set of data and ' ...
		      'move to next calibration? [RET=no quits]'], 's');
	  if ~isempty(in) && lower(in(1)) == 'y'
    ...
    if (length(in) == 0 & perblank<treject | ...
	length(in) > 0 & lower(in(1)) == 'y')
