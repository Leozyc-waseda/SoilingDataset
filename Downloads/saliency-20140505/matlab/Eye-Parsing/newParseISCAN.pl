...
while($tim > 0) {
    ...
    elsif (substr($line, 0, 26) eq "Begin: displaySurfacePatch") {
	if ($tracking == 0) { 
	    dbg("Display No Tracking Image: $image"); 
	}
	else {	    
...
while($i < $n) {
    ...
    { 
        ...
        #here we consider the actual rate in our calculation
        $toff[$i] = int($toff[$i] * $hz / $freq);
    }
    $i ++;
}
