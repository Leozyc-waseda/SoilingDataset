...
%NFMINSEARCH2 Multidimensional unconstrained nonlinear minimization (Nelder-Mead).
%   X = NFMINSEARCH2(FUN,X0) starts at X0 and finds a local minimizer X of the
%   function FUN. FUN accepts input X and returns a scalar function value 
%   F evaluated at X. X0 can be a scalar, vector or matrix. 
%
%   X = NFMINSEARCH2(FUN,X0,OPTIONS)  minimizes with the default optimization
...
%
%   X = NFMINSEARCH2(FUN,X0,OPTIONS,P1,P2,...) provides for additional
...
%
%   [X,FVAL]= NFMINSEARCH2(...) returns the value of the objective function,
%   described in FUN, at X.
%
%   [X,FVAL,EXITFLAG] = NFMINSEARCH2(...) returns a string EXITFLAG that 
...
if nargin < 2, 
   error('NFMINSEARCH2 requires at least two input arguments'); 
end
...
%=================================
% rho   - Expansion / Outside Contraction / Reflection
% chi   - Expansion
% psi   - Outside Contraction / Inside Contraction
% theta - Stochastic jump
% sigma - Shrink

%=================================
% Formulas used in simplex updates:
%=================================
% Worth Parameter Set   - v(:,end)
% Best Parameter Set    - v(:,1)
% average of the n best - xbar 
...
% Stochastic min-max    - xs     = ((max(v(:,end)) - min(v(:,end))) * theta * ((rand(size(v(:,end),1),1) - randStochOffset) * randSpread)) + v(:,end);
% Stochastic max-xbar   - xss    = ((max(v(:,end) - xbar)) * theta * ((rand(size(v(:,end),1),1) - randStochOffset) * randSpread)) + v(:,end);
% Stochastic min-xbar   - xsa    = ((min(v(:,end) - xbar)) * theta * ((rand(size(v(:,end),1),1) - randStochOffset) * randSpread)) + v(:,end);
...
if ~isfield(eoptions,'debugLevel')
    eoptions.debugLevel = 1;
end

if eoptions.debugLevel > 1
    fprintf('\n>>>>>>>RUNNING NATES FMINSEARCH 2<<<<<<<<\n');
end
...
%=================================
% useRandom - Should we randomize initial parameters?
%=================================
% 0 - Orignal Simplex
...
% Spread of random varaibles offset if 0.5 then we offset around zero
% This works by giving us:
% rand - offset
% Here rand of from 0 to 1, so setting this to 0.5 gives us a random
% number from -0.5 to 0.5 .
% if 0 we only get positive numbers
% if -1 we only get negative numbers
if isfield(eoptions,'RandOffset')
    randOffset = eoptions.RandOffset;
else
    randOffset = 0.5;
end
% The same as above but applied to stochastic jumps. probably leave this
% one be. 
if isfield(eoptions,'RandStochOffset')
    randStochOffset = eoptions.RandStochOffset;
else
    randStochOffset = 0.5;
end
...
% Durring cataclism, if we mutate, what percent of params to keep
...
% Should we use stochastic jump updates?
...
% Create a completely random parameter set
if useRandom == 3 
    y = xin;
    for i = 1:n
        ...
        y(i) = (rand(1,1) - randOffset) * new_randSpread;
    end
    y = nfminclamp(y);
    if basicOrtho == 1
...
printLog(LOC_BEST_VAL,x,'Start Up',LOGFILE);

% Create n permutations of the initial guess parameters
...
for j = 1:n
    ...
    elseif useRandom == 1 % randomize the ith parameter
        if isfield(eoptions,'ControlRand') & strcmp(eoptions.ControlRand,'yes')
            new_randSpread = randSpread(:,i);
        else
            new_randSpread = randSpread;
        end
        y(j) = (rand(1,1) - randOffset) * new_randSpread;
    elseif useRandom == 2 % randomize some of the parameters
        for i = 1:n
            doRand = rand(1,1);
            if doRand > randKeep
                if isfield(eoptions,'ControlRand') & strcmp(eoptions.ControlRand,'yes')
    ...
    elseif useRandom == 3 % randomize all parameters
        for i = 1:n
            if isfield(eoptions,'ControlRand') & strcmp(eoptions.ControlRand,'yes')
                new_randSpread = randSpread(:,i);
            else
   ...
   fv(1,j+1) = f; 
   if(eoptions.debugLevel > 1)
        printLog(f,x,'Start Up Permutation',LOGFILE);
   else
        fid = fopen(LOGFILE,'a');
...
while FUNC_EVALS < maxfun & ITERCOUNT < maxiter
   ...
   % The Reflection is not better than the best result ->   
      ...
      if fxr < fv(:,n) & EOPTIONS.ForceCataclism == 0
         ...
         fv(:,end) = fxr;
         how = 'reflect';
      % fxr >= fv(:,n) - Perform Contraction iff reflection error is not
      % better than the best error in the current set
      else 
         % Perform an OUTSIDE Contraction iff the reflexion error is worse than
         % the worse error in the current set
        ...
        % Perform an INSIDE Contraction Otherwise
        elseif EOPTIONS.ForceCataclism == 0
         ...
         % None of the main Simplex methods have yielded a better error
         % than we have int he current set. We will:
         % (A) Perform a stochastic jump if requested
         % (B) Perform a shrink
         % (C) Unleash a cataclism if we are at a minimum
         if strcmp(how,'shrink') & EOPTIONS.ForceCataclism == 0
            % Try a stochastic jump - NATE
            % Try three types of stochastic jumps in order. If one of them
            % produces a result better than the worst result in the current
            % set, replace the worst parameter and go back to the begining
            % with a new reflexion.
            % We jump from the worst point since moving in the direction of
            % the best point has yielded little so far
            if strcmp(eoptions.UseStoch,'yes')
              
                xs = nfminclamp(((max(v(:,end)) - min(v(:,end))) * theta * ((rand(size(v(:,end),1),1) - randStochOffset) * randSpread)) + v(:,end));
                ...
                if fxs < fv(:,end) 
                    ...
                    %xss = nfminclamp(((max(v(:,end) - xbar)) * theta * ((rand(size(v(:,end),1),1) - randStochOffset) * randSpread)) + v(:,end));
                    
                    xss = nfminclamp(((max(v(:,end) - xbar)) * theta * ((rand(size(v(:,end),1),1) - randStochOffset) * randSpread)) + xbar);
                    ...
                    if fxss < fv(:,end)
                        ...
                        %xsa = nfminclamp(((min(v(:,end) - xbar)) * theta * ((rand(size(v(:,end),1),1) - randStochOffset) * randSpread)) + v(:,end));
                       
                        xsa = nfminclamp(((min(v(:,end) - xbar)) * theta * ((rand(size(v(:,end),1),1) - randStochOffset) * randSpread)) + xbar);
        ...
        % Nothing worked so we try a shrink and go back to the top with a
        % reflection
        if strcmp(how,'shrink') & EOPTIONS.ForceCataclism == 0  
   ...
   if shrink == 1
    ...
    if ( bsCount == max_bsCount & useCat == 1 ) | EOPTIONS.ForceCataclism == 1
        ...
        % MUTATE the parameters by randomly replacing n% of the values in
        % each parameter set with a bounded random number
            ...
            for i = 1 : size(v,1)
                for j = 1 : size(v,2)
                    ...
                    if doRand > randKeepMut
                        ...
                        v(i,j) = (rand(1,1) - randOffset) * new_randSpread;
                    end
                end
            end
        else
       % DEATH to n% of parameter sets. Thus, n% of parameter sets will be
       % deleted and replaced with a new random set
            catType = 'death';
            LASTCAT = catType;
            for j = 1 : size(v,2)
                doRand = rand(1,1);
        ...
        for j = 1 : size(v,2)
            ...
            if eoptions.debugLevel > 1
                printLog(fm ,x,['Caticlism ' catType],LOGFILE);
            end
...
function X = printLog(fval,x,comment,logfile)

fid = fopen(logfile,'a');
fprintf(fid,'%s : Error %64.64f\n',comment,fval); 
fprintf(fid,'X = [ ');
for i = 1 : size(x,1)
	fprintf(fid,'%64.64f ... \n',x(i,1));
end
