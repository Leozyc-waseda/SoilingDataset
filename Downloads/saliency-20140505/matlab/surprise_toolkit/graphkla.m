...
function Z = graphkla(a1,a2,interval,grain)

matsize = interval/grain;

Z = zeros(matsize);
fprintf('Computing gamma kl values\n');
x = 1;
for b1 = 0.1: grain: interval
    fprintf('.');
    y = 1;
    for b2 = 0.1: grain: interval
        Z(x,y) = klgamma(a1,a2,b1,b2);
        y = y + 1;
    end
    x = x + 1;
end
...
axis([0 matsize 0 matsize 0 100])
%set(gca,'ZTickLabel','Negative||Positive');
%set(gca,'PlotBoxAspectRatio',[2.5 2.5 1]);
xlabel('b Axis');
ylabel('b` Axis');
zlabel('Surprise Value');
title('Surprise by beta values');
