function plotMainSeq3(evs, params, flds)
    Nevs = numel(evs);
    if nargin < 3
        xfld = 'amp'; yfld = 'pvel'; zfld = 'interval'; 
    else
        xfld = flds{1}; yfld = flds{2}; zfld = flds{3};
    end
    
    sfld = 'type';
    eb = struct('xyz', [evs.(xfld); evs.(yfld); evs.(zfld)], ...
        'status', [evs.type], 'len', Nevs);
    plotcolor3(eb, 'xyz', '.', false);
    
    function plotcolor3(data,request,linestyle,do_connect)
...
for ii = 1:length(code_nums) 
    ...
    for jj = 1:length(eb)
        if do_connect
            ...
            plot3(xdat(range),ydat(range),zdat(range), [corder{ii} linestyle], ...
                'markersize',2);
